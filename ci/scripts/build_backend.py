...
def cmake(backend,target,mode):
    system=platform.system().lower(); b=ROOT/'build'/f'ci-{system}-{backend}'
    if system=='windows':
        ...
        if backend=='cuda': args += ['-DSD_CUDA=ON',f"-DCMAKE_CUDA_ARCHITECTURES={';'.join(target['cuda'])}"]
            ...
            arch=target['oneapi'][0]; args += ['-DCMAKE_C_COMPILER=icx','-DCMAKE_CXX_COMPILER=icpx','-DGGML_CCACHE=OFF','-DGGML_OPENMP=OFF','-DSD_SYCL=ON','-DGGML_SYCL_TARGET=INTEL','-DGGML_SYCL_F16=ON','-DGGML_SYCL_DNN=ON','-DGGML_SYCL_SUPPORT_LEVEL_ZERO=ON']
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--backend',required=True); p.add_argument('--mode',default='compile'); p.add_argument('--target-json',required=True); a=p.parse_args(); target=json.loads(a.target_json); cmake(a.backend,target,a.mode)
