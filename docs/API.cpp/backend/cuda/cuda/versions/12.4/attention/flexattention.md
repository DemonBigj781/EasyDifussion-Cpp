...
This repository's FlexAttention is the high-resolution visual-token-selection policy. It sums host float attention across heads, normalizes, thresholds, adaptive-max-pools, then expands a host mask. CUDA 12.4 has no separate device implementation.
