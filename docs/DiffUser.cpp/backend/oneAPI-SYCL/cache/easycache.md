...
When SYCL executes the model, EasyCache remains a sampler/runtime policy. DiffUser.cpp backend work should cover only SYCL-specific transfers, synchronization, or future device-resident acceleration—not move the generic EasyCache algorithm into the backend.