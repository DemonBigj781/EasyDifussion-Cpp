...
struct LoraModel : public GGMLRunner {
    ...
    SDVersion model_version       = VERSION_COUNT;
    ...
    LoraModel(const std::string& lora_id,
        ...
        : GGMLRunner(backend, manager), lora_id(lora_id), file_path(file_path), model_manager(std::move(manager)), params_backend(params_backend_), model_version(version) {
        prefix = "lora." + prefix;
        ...
        if (actual_out_dim != expected_out_dim) {
            ...
            if (warned_incompatible_model_tensors.insert(model_tensor_name).second) {
                LOG_WARN("skip incompatible LoRA tensors for |%s|: output dim %lld != model dim %lld",
                         model_tensor_name.c_str(),
                         static_cast<long long>(actual_out_dim),
                         static_cast<long long>(expected_out_dim));
            }
            return nullptr;
        }
    ...
    void stat(bool at_runntime = false, SDVersion version = VERSION_COUNT) {
        size_t total_lora_tensors_count   = 0;
        size_t applied_lora_tensors_count = 0;
        size_t skipped_lora_tensors_count = 0;
        size_t expected_unused_count      = 0;
        const SDVersion effective_version = version == VERSION_COUNT ? model_version : version;

        auto is_expected_sdxl_penultimate_skip = [&](const std::string& name) {
            return sd_version_is_sdxl(effective_version) &&
                   name.find("lora.cond_stage_model.transformer.text_model.encoder.layers.11.") != std::string::npos;
        };
        ...
        for (auto& kv : lora_tensors) {
            ...
            if (skipped_incompatible_lora_tensors.find(kv.first) != skipped_incompatible_lora_tensors.end()) {
                ...
                if (is_expected_sdxl_penultimate_skip(kv.first)) {
                    expected_unused_count++;
                    LOG_DEBUG("skipping SDXL CLIP-L final-layer LoRA tensor |%s| (penultimate encoder output is active)",
                              kv.first.c_str());
                } else if (!at_runntime) {
        ...
        const size_t compatible_lora_tensors_count = total_lora_tensors_count - skipped_lora_tensors_count;
        const size_t unexpected_unused_count = compatible_lora_tensors_count - applied_lora_tensors_count - expected_unused_count;
        if (!at_runntime && unexpected_unused_count > 0) {
        ...
        } else if (!at_runntime && expected_unused_count > 0) {
            LOG_INFO("(%lu / %lu) active LoRA tensors applied; %lu SDXL CLIP-L final-layer tensors intentionally skipped, lora_file_path = %s",
                     applied_lora_tensors_count,
                     compatible_lora_tensors_count - expected_unused_count,
                     expected_unused_count,
                     file_path.c_str());
        } else {
