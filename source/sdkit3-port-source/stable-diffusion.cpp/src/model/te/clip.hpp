...
public:
    ...
    CLIPTextModel(CLIPVersion version = OPENAI_CLIP_VIT_L_14,
        ...
        n_token = max_position_embeddings;
...
struct CLIPTextModelRunner : public GGMLRunner {
    ...
    CLIPTextModelRunner(ggml_backend_t backend,
        ...
        int32_t max_position_embeddings = 77;
        const std::string position_embedding_name = prefix + ".embeddings.position_embedding.weight";
        auto position_embedding = tensor_storage_map.find(position_embedding_name);
        if (position_embedding != tensor_storage_map.end() && position_embedding->second.n_dims >= 2) {
            const int64_t detected_positions = position_embedding->second.ne[1];
            if (detected_positions >= 2 && detected_positions <= 4096) {
                max_position_embeddings = static_cast<int32_t>(detected_positions);
            }
        }
        if (max_position_embeddings != 77) {
            LOG_INFO("LongCLIP detected for %s (native context: %d tokens)",
                     prefix.c_str(),
                     max_position_embeddings);
        }
        model = CLIPTextModel(version,
                              with_final_ln,
                              force_clip_f32,
                              proj_in,
                              max_position_embeddings);
