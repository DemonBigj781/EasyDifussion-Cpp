...
public:
    ...
    ggml_tensor* forward(GGMLRunnerContext* ctx,
        ...
        // ControlNet-LITE returns five encoder-side controls instead of the
        // thirteen decoder-side residuals produced by standard ControlNet.
        const bool lite_controls          = controls.size() == 5;
        int lite_control_offset            = 0;
        ...
        for (int i = 0; i < len_mults; i++) {
            ...
            for (int j = 0; j < num_res_blocks; j++) {
                input_block_idx += 1;
                if (lite_controls &&
                    (input_block_idx == 1 || input_block_idx == 4 ||
                     input_block_idx == 7 || input_block_idx == 10)) {
                    auto cs = ggml_ext_scale(ctx->ggml_ctx,
                                             controls[static_cast<size_t>(lite_control_offset++)],
                                             control_strength,
                                             true);
                    h = ggml_add(ctx->ggml_ctx, h, cs);
                }
                std::string name = "input_blocks." + std::to_string(input_block_idx) + ".0";
        ...
        // middle_block
        if (lite_controls) {
            GGML_ASSERT(lite_control_offset == 4);
            auto cs = ggml_ext_scale(ctx->ggml_ctx,
                                     controls[static_cast<size_t>(lite_control_offset++)],
                                     control_strength,
                                     true);
...
struct UNetModelRunner : public DiffusionModelRunner {
    ...
    UnetModelBlock unet;
    LLLiteModel lllite;
    const void* last_lllite_condition_data = nullptr;
    int last_lllite_step                   = -1;
    ...
    void get_param_tensors(std::map<std::string, ggml_tensor*>& tensors, const std::string& prefix) override {
        ...
        lllite.get_param_tensors(tensors);
    }
    ...
    ggml_cgraph* build_graph(const sd::Tensor<float>& x_tensor,
                             ...
                             float control_strength                                = 0.f,
                             const sd::Tensor<float>& lllite_condition_tensor      = {},
        ...
        if (lllite_condition != nullptr && !lllite.empty() && lllite_strength != 0.f && lllite_steps > 0) {
            const int step_index = std::max(0, lllite_step - 1);
            const int start_step = lllite_start_percent > 0.f
                                       ? static_cast<int>(std::floor(lllite_steps * lllite_start_percent * 0.01f))
                                       : 0;
            const int end_step = lllite_end_percent > 0.f
                                     ? static_cast<int>(std::floor(lllite_steps * lllite_end_percent * 0.01f))
                                     : lllite_steps;
            if (step_index >= start_step && step_index < end_step) {
                runner_ctx.attention_qkv_patch = [this, &runner_ctx, lllite_condition, lllite_strength](const std::string& name, ggml_tensor* value) {
                    return lllite.forward(&runner_ctx, name, value, lllite_condition, lllite_strength);
                };
            }
        }
    ...
    sd::Tensor<float> compute(int n_threads,
                              ...
                              float control_strength                         = 0.f,
                              const sd::Tensor<float>& lllite_condition      = {},
                              float lllite_strength                          = 0.f,
                              int lllite_step                                = 0,
                              int lllite_steps                               = 0,
                              float lllite_start_percent                     = 0.f,
        ...
        // c_concat: [N, in_channels, h, w] or [1, in_channels, h, w]
        // y: [N, adm_in_channels] or [1, adm_in_channels]
        if (!lllite_condition.empty()) {
            const void* condition_data = lllite_condition.data();
            const bool restarted       = lllite_step <= 1 && last_lllite_step > 1;
            if (condition_data != last_lllite_condition_data || restarted) {
                free_cache_ctx_and_buffer();
                last_lllite_condition_data = condition_data;
            }
            last_lllite_step = lllite_step;
        }
