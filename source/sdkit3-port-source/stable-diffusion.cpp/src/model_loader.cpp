...
void ModelLoader::convert_controlnet_tensors_name(SDVersion version, const std::string& prefix) {
    String2TensorStorage new_map;

    for (auto& [_, tensor_storage] : tensor_storage_map) {
        if (starts_with(tensor_storage.name, prefix)) {
            tensor_storage.name = convert_controlnet_tensor_name(tensor_storage.name.substr(prefix.size()), version);
        }
...
SDVersion ModelLoader::get_sd_version() {
    ...
    for (auto& [name, tensor_storage] : tensor_storage_map) {
        ...
        if (tensor_storage.name.find("model.diffusion_model.pos_frequencies") != std::string::npos &&
            ...
            tensor_storage_map.find("model.diffusion_model.blocks.47.mod_y.weight") != tensor_storage_map.end()) {
            return VERSION_MOCHI;
        }
