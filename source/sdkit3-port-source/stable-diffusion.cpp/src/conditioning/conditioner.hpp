...
struct SDCondition {
    ...
    bool empty() const {
        if (!c_crossattn.empty() || !c_vector.empty() || !c_concat.empty() || !c_ip_adapter.empty() ||
...
struct CLIPTextEncoding {
    sd::Tensor<float> hidden_states;
    sd::Tensor<float> pooled;
};

// Encode each text encoder at its native positional-embedding length. This is
// important for LongCLIP (normally 248 positions), while preserving the normal
// 77-token chunking of stock CLIP-L/CLIP-G models.
static inline CLIPTextEncoding encode_clip_token_chunks(CLIPTextModelRunner& runner,
                                                        ...
                                                        int clip_skip,
                                                        bool return_pooled,
                                                        bool zero_after_eos = false) {
    CLIPTextEncoding result;
    const size_t chunk_len = static_cast<size_t>(runner.model.n_token);
    if (chunk_len == 0 || tokens.empty() || tokens.size() != weights.size() || tokens.size() % chunk_len != 0) {
        LOG_ERROR("invalid CLIP token stream: %zu tokens, %zu weights, native context %zu",
                  tokens.size(),
                  weights.size(),
                  chunk_len);
        return result;
    ...
    const size_t chunk_count = tokens.size() / chunk_len;
    for (size_t chunk_idx = 0; chunk_idx < chunk_count; ++chunk_idx) {
        const size_t begin = chunk_idx * chunk_len;
        const size_t end   = begin + chunk_len;
        std::vector<int> chunk_tokens(tokens.begin() + begin, tokens.begin() + end);
        std::vector<float> chunk_weights(weights.begin() + begin, weights.begin() + end);

        auto eos = std::find(chunk_tokens.begin(), chunk_tokens.end(), eos_token_id);
        const size_t max_token_idx = std::min<size_t>(std::distance(chunk_tokens.begin(), eos), chunk_len - 1);
        if (zero_after_eos && eos != chunk_tokens.end()) {
            std::fill(std::next(eos), chunk_tokens.end(), 0);
        }

        sd::Tensor<int32_t> input_ids({static_cast<int64_t>(chunk_len)}, chunk_tokens);
        auto chunk_hidden_states = runner.compute(n_threads,
                                                  input_ids,
                                                  num_custom_embeddings,
                                                  custom_embeddings_data,
                                                  max_token_idx,
                                                  false,
                                                  clip_skip,
                                                  false,
        ...
        if (result.hidden_states.empty()) {
            result.hidden_states = std::move(chunk_hidden_states);
        } else {
            result.hidden_states = sd::ops::concat(result.hidden_states, chunk_hidden_states, 1);
        }

        if (return_pooled && chunk_idx == 0) {
            result.pooled = runner.compute(n_threads,
                                           input_ids,
                                           num_custom_embeddings,
                                           custom_embeddings_data,
                                           max_token_idx,
                                           ...
                                           true,
                                           true);
            GGML_ASSERT(!result.pooled.empty());
        }
    }
    return result;
}
...
struct FrozenCLIPEmbedderWithCustomWords : public Conditioner {
    ...
    FrozenCLIPEmbedderWithCustomWords(ggml_backend_t backend,
        ...
        : version(version), tokenizer(sd_version_is_sd2(version) ? 0 : 49407), tokenizer2(0) {
        for (const auto& kv : orig_embedding_map) {
            ...
            tokenizer2.add_special_token(name);
        }
    ...
    std::pair<std::vector<int>, std::vector<float>> tokenize_with(CLIPTokenizer& active_tokenizer,
                                                                  std::string text,
                                                                  size_t min_length          = 0,
                                                                  size_t max_length          = 0,
        ...
        for (const auto& item : parsed_attention) {
            ...
            if (curr_text == "BREAK" && curr_weight == -1.0f) {
                ...
                // Leave room for BOS and EOS in the active encoder's native context.
                const size_t payload_length = min_length > 2 ? min_length - 2 : 75;
                size_t current_size = tokens.size();
                size_t padding_size = (payload_length - (current_size % payload_length)) % payload_length;

                if (padding_size > 0) {
                    LOG_DEBUG("BREAK token encountered, padding current chunk by %zu tokens.", padding_size);
                    tokens.insert(tokens.end(), padding_size, active_tokenizer.EOS_TOKEN_ID);
                    weights.insert(weights.end(), padding_size, 1.0f);
                }
                continue;  // Skip to the next item after handling BREAK
            }

            std::vector<int> curr_tokens = active_tokenizer.encode(curr_text, on_new_token_cb);
            tokens.insert(tokens.end(), curr_tokens.begin(), curr_tokens.end());
            weights.insert(weights.end(), curr_tokens.size(), curr_weight);
        }

        active_tokenizer.pad_tokens(tokens, &weights, nullptr, min_length, max_length, allow_overflow_expand);
    ...
    std::pair<std::vector<int>, std::vector<float>> tokenize(std::string text,
                                                             ...
                                                             size_t max_length          = 0,
                                                             bool allow_overflow_expand = true) {
        return tokenize_with(tokenizer, std::move(text), min_length, max_length, allow_overflow_expand);
    }
    ...
    SDCondition get_learned_condition_common(int n_threads,
                                             ...
                                             int height,
                                             bool zero_out_masked = false,
                                             const std::vector<int>* tokens2 = nullptr,
                                             const std::vector<float>* weights2 = nullptr) {
        ...
        auto encoded_l = encode_clip_token_chunks(*text_model,
                                                  tokens,
                                                  weights,
                                                  ...
                                                  clip_skip,
                                                  false);
        GGML_ASSERT(!encoded_l.hidden_states.empty());
        hidden_states = std::move(encoded_l.hidden_states);

        if (sd_version_is_sdxl(version)) {
            std::vector<int> fallback_tokens2;
            std::vector<float> fallback_weights2;
            if (tokens2 == nullptr || weights2 == nullptr) {
                fallback_tokens2  = tokens;
                fallback_weights2 = weights;
                const size_t native_g = static_cast<size_t>(text_model2->model.n_token);
                if (fallback_tokens2.size() % native_g != 0) {
                    fallback_tokens2.resize(native_g, tokenizer2.PAD_TOKEN_ID);
                    fallback_weights2.resize(native_g, 1.0f);
                    if (!fallback_tokens2.empty()) {
                        fallback_tokens2.back() = tokenizer2.EOS_TOKEN_ID;
                    }
                }
                tokens2  = &fallback_tokens2;
                weights2 = &fallback_weights2;
            }

            auto encoded_g = encode_clip_token_chunks(*text_model2,
                                                      *tokens2,
                                                      *weights2,
                                                      ...
                                                      true,
                                                      true);
            GGML_ASSERT(!encoded_g.hidden_states.empty());
            GGML_ASSERT(!encoded_g.pooled.empty());
            const int64_t common_tokens = std::min(hidden_states.shape()[1], encoded_g.hidden_states.shape()[1]);
            if (hidden_states.shape()[1] != encoded_g.hidden_states.shape()[1]) {
                LOG_DEBUG("aligning SDXL CLIP-L/CLIP-G sequences to %" PRId64 " tokens (%" PRId64 " vs %" PRId64 ")",
                          common_tokens,
                          hidden_states.shape()[1],
                          encoded_g.hidden_states.shape()[1]);
            }
            hidden_states = sd::ops::slice(hidden_states, 1, 0, common_tokens);
            encoded_g.hidden_states = sd::ops::slice(encoded_g.hidden_states, 1, 0, common_tokens);
            hidden_states = sd::ops::concat(hidden_states, encoded_g.hidden_states, 0);
            pooled = std::move(encoded_g.pooled);
        }
    ...
    SDCondition get_learned_condition(int n_threads,
        ...
        // Stable Video Diffusion is image-conditioned and has no text
        // encoder. Its CLIP-vision result is attached to c_vector later.
        if (!text_model) {
            return {};
        }
        ...
        const std::vector<float>* weights2    = nullptr;
        if (sd_version_is_sdxl(version)) {
            tokens_and_weights2 = tokenize_with(tokenizer2,
...
struct FluxCLIPEmbedder : public Conditioner {
    ...
    std::vector<std::pair<std::vector<int>, std::vector<float>>> tokenize(std::string text,
        ...
        if (clip_l) {
            const size_t clip_context = static_cast<size_t>(clip_l->model.n_token);
            clip_l_tokenizer.pad_tokens(clip_l_tokens, &clip_l_weights, nullptr, clip_context, clip_context, true);
        }
        if (t5) {
            t5_tokenizer.pad_tokens(t5_tokens, &t5_weights, nullptr, min_length, max_length, true);
        }
    ...
    SDCondition get_learned_condition_common(int n_threads,
        ...
        for (int chunk_idx = 0; chunk_idx < chunk_count; chunk_idx++) {
            ...
            if (chunk_idx == 0) {
                if (clip_l) {
                    size_t chunk_len_l = static_cast<size_t>(clip_l->model.n_token);
