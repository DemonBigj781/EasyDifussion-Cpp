...
public:
    ...
    std::shared_ptr<FrozenCLIPVisionEmbedder> clip_vision;  // for video conditioning or Uni-ControlNet global control
    ...
    std::shared_ptr<UniControlGlobalAdapter> uni_control_global_adapter;
    std::shared_ptr<LatentInterposerRunner> latent_interposer;
    std::shared_ptr<LatentInterposerRunner> latent_interposer_encode;
    ...
    bool init_model_loader(ModelLoader& model_loader,
                           ...
                           bool& use_control_net_lllite,
                           bool& use_latent_interposer,
                           bool& use_latent_interposer_encode,
                           bool& use_latent_interposer_decode,
                           bool& use_furception_vae) {
        const bool has_control_net_sd1_path = strlen(SAFE_STR(sd_ctx_params->control_net_sd1_path)) > 0;
        const bool has_control_net_sdxl_path = strlen(SAFE_STR(sd_ctx_params->control_net_sdxl_path)) > 0;
        if (has_control_net_sd1_path != has_control_net_sdxl_path) {
            LOG_ERROR("automatic ControlNet selection requires both control_net_sd1_path and control_net_sdxl_path");
            return false;
        }
        const bool use_automatic_control_net = has_control_net_sd1_path && has_control_net_sdxl_path;
        if (use_automatic_control_net && strlen(SAFE_STR(sd_ctx_params->control_net_path)) > 0) {
            LOG_WARN("control_net_path is ignored when the automatic SD1.x/SDXL ControlNet pair is configured");
        }
        ...
        if (!use_automatic_control_net && strlen(SAFE_STR(sd_ctx_params->control_net_path)) > 0) {
            if (!model_loader.init_from_file(sd_ctx_params->control_net_path, "controlnet.")) {
        ...
        if (strlen(SAFE_STR(sd_ctx_params->control_net_lllite_path)) > 0) {
            LOG_INFO("loading ControlNet-LLLite from '%s'", sd_ctx_params->control_net_lllite_path);
            if (!model_loader.init_from_file(sd_ctx_params->control_net_lllite_path, "lllite.")) {
                LOG_ERROR("init ControlNet-LLLite model loader from file failed: '%s'", sd_ctx_params->control_net_lllite_path);
                return false;
            }
            use_control_net_lllite = true;
        }

        if (strlen(SAFE_STR(sd_ctx_params->latent_interposer_path)) > 0) {
            LOG_INFO("loading Latent Interposer from '%s'", sd_ctx_params->latent_interposer_path);
            if (!model_loader.init_from_file(sd_ctx_params->latent_interposer_path, "latent_interposer.")) {
                LOG_ERROR("init Latent Interposer model loader from file failed: '%s'", sd_ctx_params->latent_interposer_path);
                return false;
            }
            use_latent_interposer = true;
        }

        if (strlen(SAFE_STR(sd_ctx_params->latent_interposer_encode_path)) > 0) {
            LOG_INFO("loading Encode Latent Interposer from '%s'", sd_ctx_params->latent_interposer_encode_path);
            if (!model_loader.init_from_file(sd_ctx_params->latent_interposer_encode_path, "latent_interposer_encode.")) {
                LOG_ERROR("init Encode Latent Interposer loader failed: '%s'", sd_ctx_params->latent_interposer_encode_path);
                return false;
            }
            use_latent_interposer_encode = true;
        }

        if (strlen(SAFE_STR(sd_ctx_params->latent_interposer_decode_path)) > 0) {
            LOG_INFO("loading Decode Latent Interposer from '%s'", sd_ctx_params->latent_interposer_decode_path);
            if (!model_loader.init_from_file(sd_ctx_params->latent_interposer_decode_path, "latent_interposer_decode.")) {
                LOG_ERROR("init Decode Latent Interposer loader failed: '%s'", sd_ctx_params->latent_interposer_decode_path);
                return false;
            }
            use_latent_interposer_decode = true;
        }

        if (strlen(SAFE_STR(sd_ctx_params->furception_vae_path)) > 0) {
            LOG_INFO("loading Furception v1 latent VAE from '%s'", sd_ctx_params->furception_vae_path);
            if (!model_loader.init_from_file(sd_ctx_params->furception_vae_path, "furception_vae.")) {
                LOG_ERROR("init Furception VAE model loader from file failed: '%s'", sd_ctx_params->furception_vae_path);
                return false;
            }
            use_furception_vae = true;
        }
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
        ...
        bool use_control_net_lllite       = false;
        bool use_latent_interposer        = false;
        bool use_latent_interposer_encode = false;
        bool use_latent_interposer_decode = false;
        bool use_furception_vae           = false;
        const bool use_automatic_control_net =
            strlen(SAFE_STR(sd_ctx_params->control_net_sd1_path)) > 0 &&
            strlen(SAFE_STR(sd_ctx_params->control_net_sdxl_path)) > 0;
        ...
        if (!init_model_loader(model_loader,
                               ...
                               use_audio_vae,
                               use_control_net,
                               use_control_net_lllite,
                               use_latent_interposer,
                               use_latent_interposer_encode,
                               use_latent_interposer_decode,
                               use_furception_vae)) {
            return false;
        }
        ...
        if (use_automatic_control_net) {
            const char* selected_control_net_path = nullptr;
            const char* selected_control_net_name = nullptr;
            if (sd_version_is_sd1(version)) {
                selected_control_net_path = sd_ctx_params->control_net_sd1_path;
                selected_control_net_name = "Uni-ControlNet";
            } else if (sd_version_is_sdxl(version)) {
                selected_control_net_path = sd_ctx_params->control_net_sdxl_path;
                selected_control_net_name = "ControlNet Union";
            } else {
                LOG_ERROR("automatic ControlNet selection supports only SD1.x and SDXL base checkpoints; detected %s",
                          model_version_to_str[version]);
                return false;
            }
            LOG_INFO("base checkpoint weights identify %s; loading %s from '%s'",
                     model_version_to_str[version],
                     selected_control_net_name,
                     selected_control_net_path);
            if (!model_loader.init_from_file(selected_control_net_path, "controlnet.")) {
                LOG_ERROR("init automatically selected %s model loader from file failed: '%s'",
                          selected_control_net_name,
                          selected_control_net_path);
                return false;
            }
        ...
        size_t clip_vision_params_mem_size   = 0;
        size_t unet_params_mem_size         = 0;
        size_t vae_params_mem_size          = 0;
        size_t ip_adapter_params_mem_size    = 0;
        control_net_params_mem_size         = 0;
        ...
        {
            SDBackendModule conditioner_backend_module = SDBackendModule::TE;
            if (!ensure_backend_pair(SDBackendModule::TE) ||
                !ensure_backend_pair(SDBackendModule::LLM) ||
            ...
            if (sd_version_is_sd3(version)) {
                ...
                cond_stage_model = std::make_shared<LLMEmbedder>(backend_for(conditioner_backend_module),
                                                                 tensor_storage_map,
                                                                 version,
                                                                 "",
                                                                 false,
                ...
                // SVD has no text encoder. The empty FrozenCLIP wrapper keeps
                // the common conditioning path valid, while the embedded
                // OpenCLIP vision encoder supplies c_vector.
                cond_stage_model = std::make_shared<FrozenCLIPEmbedderWithCustomWords>(
                    backend_for(SDBackendModule::TE), tensor_storage_map,
                    std::map<std::string, std::string>{}, version, model_manager);
                ...
                clip_vision = std::make_shared<FrozenCLIPVisionEmbedder>(backend_for(SDBackendModule::CLIP_VISION),
                                                                         ...
                                                                         model_manager);
                clip_vision->set_max_graph_vram_bytes(max_graph_vram_bytes_for_module(SDBackendModule::CLIP_VISION));
                if (!register_runner_params("SVD CLIP vision",
                                            clip_vision,
                                            SDBackendModule::CLIP_VISION,
                                            &clip_vision_params_mem_size)) {
                    return false;
            ...
            cond_stage_model->set_max_graph_vram_bytes(max_graph_vram_bytes_for_module(conditioner_backend_module));
            ...
            if (use_latent_interposer) {
                if (!ensure_backend_pair(SDBackendModule::LATENT_INTERPOSER)) {
                    return false;
                }
                latent_interposer = std::make_shared<LatentInterposerRunner>(backend_for(SDBackendModule::LATENT_INTERPOSER),
                                                                             tensor_storage_map,
                                                                             "latent_interposer",
                                                                             model_manager);
                if ((latent_interposer->in_channels != 4 && latent_interposer->in_channels != 16) ||
                    latent_interposer->out_channels != get_latent_channel()) {
                    LOG_ERROR("Latent Interposer channel mismatch: model is %" PRId64 " -> %" PRId64
                              ", but supported sources have 4 or 16 channels and the loaded diffusion model requires %d output channels",
                              latent_interposer->in_channels,
                              latent_interposer->out_channels,
                              get_latent_channel());
                    return false;
                }
                if (!register_runner_params("Latent Interposer",
                                            latent_interposer,
            ...
            if (use_latent_interposer_encode) {
                if (!ensure_backend_pair(SDBackendModule::LATENT_INTERPOSER_ENCODE)) {
                    return false;
                }
                latent_interposer_encode = std::make_shared<LatentInterposerRunner>(backend_for(SDBackendModule::LATENT_INTERPOSER_ENCODE),
                                                                                    tensor_storage_map,
                                                                                    "latent_interposer_encode",
                                                                                    model_manager);
                if ((latent_interposer_encode->in_channels != 4 && latent_interposer_encode->in_channels != 16) ||
                    latent_interposer_encode->out_channels != get_latent_channel()) {
                    LOG_ERROR("Encode Interposer channel mismatch: converter is %" PRId64 " -> %" PRId64
                              ", but the diffusion model requires %d output channels",
                              latent_interposer_encode->in_channels,
                              latent_interposer_encode->out_channels,
                              get_latent_channel());
                    return false;
                }
                if (!register_runner_params("Encode Latent Interposer",
                                            latent_interposer_encode,
            ...
            if (use_latent_interposer_decode) {
                if (!ensure_backend_pair(SDBackendModule::LATENT_INTERPOSER_DECODE)) {
                    return false;
                }
                latent_interposer_decode = std::make_shared<LatentInterposerRunner>(backend_for(SDBackendModule::LATENT_INTERPOSER_DECODE),
                                                                                    tensor_storage_map,
                                                                                    "latent_interposer_decode",
                                                                                    model_manager);
                if (latent_interposer_decode->in_channels != get_latent_channel() ||
                    (latent_interposer_decode->out_channels != 4 && latent_interposer_decode->out_channels != 16)) {
                    LOG_ERROR("Decode Interposer channel mismatch: converter is %" PRId64 " -> %" PRId64
                              ", but the diffusion model supplies %d input channels",
                              latent_interposer_decode->in_channels,
                              latent_interposer_decode->out_channels,
                              get_latent_channel());
                    return false;
                }
                if (!register_runner_params("Decode Latent Interposer",
                                            latent_interposer_decode,
            ...
            if (strlen(SAFE_STR(sd_ctx_params->ip_adapter_path)) > 0) {
                ...
                ip_adapter = std::make_shared<IPAdapter::IPAdapterRunner>(backend_for(SDBackendModule::IP_ADAPTER),
                                                                          tensor_storage_map,
                                                                          "ip_adapter",
                                                                          model_manager);
                ip_adapter->set_max_graph_vram_bytes(max_graph_vram_bytes_for_module(SDBackendModule::IP_ADAPTER));
                if (!register_runner_params("IP-Adapter",
                                            ip_adapter,
                                            SDBackendModule::IP_ADAPTER,
                                            &ip_adapter_params_mem_size)) {
                    return false;
                }
            }

            if (!ensure_backend_pair(SDBackendModule::VAE) ||
                !ensure_backend_pair(SDBackendModule::VAE_ENCODE) ||
                !ensure_backend_pair(SDBackendModule::VAE_DECODE)) {
                return false;
            }

            if (use_furception_vae) {
                furception_vae = std::make_shared<AutoEncoderKL>(backend_for(SDBackendModule::VAE),
                                                                  tensor_storage_map,
                                                                  "furception_vae",
                                                                  false,
                                                                  false,
                                                                  VERSION_SD1,
                                                                  model_manager);
                furception_vae->set_max_graph_vram_bytes(max_graph_vram_bytes_for_module(SDBackendModule::VAE));
                if (!register_runner_params("Furception VAE",
                                            furception_vae,
                                            SDBackendModule::VAE,
                                            &vae_params_mem_size)) {
                    return false;
            ...
            if (strlen(SAFE_STR(sd_ctx_params->vae_path)) > 0 && vae_format != SD_VAE_FORMAT_AUTO) {
            ...
            if (use_control_net) {
                ...
                const auto& control_tensors = model_loader.get_tensor_storage_map();
                auto has_control_tensor = [&](const char* name) {
                    return control_tensors.find(name) != control_tensors.end();
                };
                const bool has_uni_global = has_control_tensor("global_adapter.norm1.weight");
                const bool has_uni_local = has_control_tensor("feature_extractor.pre_extractor.0.weight");
                const bool has_control_lite = has_control_tensor("input_blocks_control.0.0.weight");
                const bool has_spatial_control =
                    has_control_tensor("input_blocks.0.0.weight") ||
                    has_uni_local || has_control_lite;

                // These architectures have fixed SD1.5 channel layouts. Reject
                // an incompatible base checkpoint before ControlNetBlock's
                // topology assertions are reached so an API error cannot abort
                // the entire native backend process.
                if ((has_uni_local || has_control_lite) && !sd_version_is_sd1(version)) {
                    LOG_ERROR("Uni-ControlNet and ControlNet-LITE spatial adapters require an SD1.x checkpoint");
                    return false;
                }
                ...
                if (has_uni_global) {
                    if (ip_adapter != nullptr) {
                        LOG_ERROR("IP-Adapter and Uni-ControlNet global conditioning require different CLIP vision models and cannot share one context");
                        return false;
                    }
                    if (!sd_version_is_sd1(version)) {
                        LOG_ERROR("Uni-ControlNet global adapter supports SD1.x checkpoints only");
                        return false;
                    }
                    const bool has_clip_vision = has_control_tensor(
                        "cond_stage_model.transformer.vision_model.embeddings.patch_embedding.weight");
                    if (!has_clip_vision) {
                        LOG_ERROR("Uni-ControlNet global adapter requires OpenAI CLIP ViT-L/14 via clip_vision_path");
                        return false;
                    }
                    if (!ensure_backend_pair(SDBackendModule::CLIP_VISION)) {
                        return false;
                    ...
                    clip_vision->set_max_graph_vram_bytes(max_graph_vram_bytes_for_module(SDBackendModule::CLIP_VISION));
                    if (!register_runner_params("Uni-ControlNet CLIP vision",
                                                clip_vision,
                                                SDBackendModule::CLIP_VISION,
                                                &clip_vision_params_mem_size)) {
                        return false;
                    ...
                    uni_control_global_adapter = std::make_shared<UniControlGlobalAdapter>(
                        backend_for(SDBackendModule::CONTROL_NET),
                        control_tensors,
                        "global_adapter",
                        model_manager);
                    ...
                    LOG_INFO("detected Uni-ControlNet global/content adapter (OpenAI CLIP ViT-L/14)");
                }

                if (!has_spatial_control && !has_uni_global) {
                    LOG_ERROR("control checkpoint contains no supported ControlNet tensors");
                    return false;
                }
            }
            ...
            if (sd_ctx_params->flash_attn || sd_ctx_params->diffusion_flash_attn || sd_ctx_params->diffusion_sage_attn) {
            ...
            if (sd_ctx_params->diffusion_sage_attn) {
                LOG_INFO("Preferring native SageAttention SM80 kernels in the diffusion model");
                diffusion_model->set_sage_attention_enabled(true);
                if (high_noise_diffusion_model) {
                    high_noise_diffusion_model->set_sage_attention_enabled(true);
                }
            }
        }
...

        {
            ...
            if (!add_params_memory(text_encoder_params_mem_size, SDBackendModule::TE) ||
                !add_params_memory(clip_vision_params_mem_size, SDBackendModule::CLIP_VISION) ||
                ...
                !add_params_memory(ip_adapter_params_mem_size, SDBackendModule::IP_ADAPTER) ||
            ...
            LOG_INFO(
                ...
                "text_encoders %.2fMB(%s), clip_vision %.2fMB(%s), diffusion_model %.2fMB(%s), ip_adapter_projection %.2fMB(%s), vae %.2fMB(%s), controlnet %.2fMB(%s), extensions %.2fMB(%s)",
                ...
                clip_vision_params_mem_size / 1024.0 / 1024.0,
                params_memory_location(clip_vision_params_mem_size, SDBackendModule::CLIP_VISION),
                unet_params_mem_size / 1024.0 / 1024.0,
                params_memory_location(unet_params_mem_size, SDBackendModule::DIFFUSION),
                ip_adapter_params_mem_size / 1024.0 / 1024.0,
                params_memory_location(ip_adapter_params_mem_size, SDBackendModule::IP_ADAPTER),
    ...
    bool compute_ip_adapter_tokens(const sd_image_t& image, float strength) {
        ip_adapter_tokens        = {};
        ...
        if (auto unet = std::dynamic_pointer_cast<UNetModelRunner>(diffusion_model);
            unet != nullptr && ip_adapter->expected_context_dim() != unet->config.context_dim) {
            LOG_ERROR("IP-Adapter/checkpoint shape mismatch: adapter outputs context width %lld, but the diffusion model expects %d. Select an adapter built for this checkpoint family.",
                      static_cast<long long>(ip_adapter->expected_context_dim()),
                      unet->config.context_dim);
            ip_adapter_tokens = {};
            return false;
        }
    ...
    void preview_image(int step,
        ...
        if (preview_mode == PREVIEW_VAE || preview_mode == PREVIEW_TAE) {
            ...
            if (preview_vae) {
                ...
                vae_latents = diffusion_to_decode_vae_latents(_latents, "Decode preview");
                if (!vae_latents.empty()) {
    ...
    void report_sample_progress(int step,
        ...
        if (sd::preview::sample_step_is_complete(step, total_steps, terminal_sigma_is_zero)) {
            ...
            float step_seconds = last_progress_us != nullptr && *last_progress_us > 0
                                     ...
                                     : 0.f;
            sd_progress_cb_t sample_progress_cb = sd_get_sample_progress_callback();
            if (sample_progress_cb != nullptr) {
                sample_progress_cb(showstep,
                                   (int)total_steps,
                                   step_seconds,
                                   sd_get_sample_progress_callback_data());
            } else {
    ...
    void compute_sample_controls(const sd::Tensor<float>& control_image,
        ...
        if (control_image.empty() || control_net == nullptr ||
            control_type == CONTROL_NET_TYPE_GLOBAL) {
            return;
        }
    ...
    sd::Tensor<float> sample(const std::shared_ptr<DiffusionModelRunner>& work_diffusion_model,
                             ...
                             const sd::Tensor<float>& control_net_lllite_image = {},
                             float control_net_lllite_strength                 = 0.f,
                             float control_net_lllite_start_percent            = 0.f,
                             float control_net_lllite_end_percent              = 0.f,
        ...
        auto denoise = [&](const sd::Tensor<float>& x, float sigma, int step) -> sd::guidance::GuiderOutput {
            ...
            const int step_index              = std::max(0, std::abs(step) - 1);
            const int ip_adapter_start_step   = static_cast<int>(std::floor(steps * std::clamp(ip_adapter_start_percent, 0.f, 100.f) * 0.01f));
            const int ip_adapter_end_step     = static_cast<int>(std::ceil(steps * std::clamp(ip_adapter_end_percent, 0.f, 100.f) * 0.01f));
            const float active_ip_adapter_strength = step_index >= ip_adapter_start_step &&
                                                             step_index < ip_adapter_end_step
                                                         ? ip_adapter_strength
                                                         : 0.f;
            ...
            auto run_condition = [&](const SDCondition& condition,
                ...
                if (sd_version_is_unet(version) || version == VERSION_SVD) {
                    ...
                    } else if (animatediff_loaded && noised_input.dim() >= 4 && noised_input.shape()[3] > 1) {
                        nvf = static_cast<int>(noised_input.shape()[3]);
                    }
                    UNetDiffusionExtra unet_extra{nvf, &controls, control_strength};
                    unet_extra.lllite_condition = control_net_lllite_image.empty() ? nullptr : &control_net_lllite_image;
                    unet_extra.lllite_strength = control_net_lllite_strength;
                    unet_extra.lllite_step = step;
                    unet_extra.lllite_steps = static_cast<int>(steps);
                    unet_extra.lllite_start_percent = control_net_lllite_start_percent;
                    unet_extra.lllite_end_percent = control_net_lllite_end_percent;
                    const auto& ip_tokens = use_uncond_ip ? ip_adapter_uncond_tokens : ip_adapter_tokens;
                    if (!ip_tokens.empty()) {
                        unet_extra.ip_context = &ip_tokens;
                        unet_extra.ip_scale   = active_ip_adapter_strength;
                    }
    ...
    int get_diffusion_model_down_factor() {
        ...
        if (sd_version_is_dit(version)) {
            if (sd_version_is_wan(version) || sd_version_is_mochi(version) ||
                sd_version_is_lingbot_video(version) || sd_version_is_minimax_h3(version)) {
                down_factor = 2;
            } else {
                down_factor = 1;
            }
    ...
    sd_latent_source_t target_latent_family() const {
        if (sd_version_is_sd1(version)) {
            return SD_LATENT_SOURCE_V1;
        }
        if (sd_version_is_sdxl(version)) {
            return SD_LATENT_SOURCE_XL;
        }
        if (sd_version_is_sd3(version)) {
            return SD_LATENT_SOURCE_V3;
        }
        if (sd_version_is_flux(version)) {
            return SD_LATENT_SOURCE_FX;
        }
        return SD_LATENT_SOURCE_NONE;
    }

    static int latent_source_channels(sd_latent_source_t source) {
        return (source == SD_LATENT_SOURCE_V3 || source == SD_LATENT_SOURCE_FX) ? 16 : 4;
    }

    sd::Tensor<float> convert_interposed_noise(const sd::Tensor<float>& source,
                                                const sd::Tensor<float>& target,
                                                sd_latent_source_t source_family) {
        if (source.empty() || target.empty() || source.dim() != 4 || target.dim() != 4) {
            return {};
        }

        const sd_latent_source_t target_family = target_latent_family();
        if (target_family == SD_LATENT_SOURCE_NONE) {
            LOG_ERROR("Latent Interposer target model family is unsupported");
            return {};
        }
        if (source_family == target_family) {
            if (latent_interposer != nullptr) {
                LOG_WARN("ignoring a Latent Interposer converter for a same-family source");
            }
            if (source.shape() != target.shape()) {
                LOG_ERROR("same-family latent source shape %s does not match sampler noise shape %s",
                          sd::tensor_shape_to_string(source.shape()).c_str(),
                          sd::tensor_shape_to_string(target.shape()).c_str());
                return {};
            }
            return source;
        }
        if (latent_interposer == nullptr) {
            LOG_ERROR("cross-family latent source requires a city96 interposer model");
            return {};
        }
        if (latent_interposer->in_channels != source.shape()[2] ||
            latent_interposer->out_channels != target.shape()[2]) {
            LOG_ERROR("Latent Interposer model channels %" PRId64 " -> %" PRId64
                      " do not match request channels %" PRId64 " -> %" PRId64,
                      latent_interposer->in_channels,
                      latent_interposer->out_channels,
                      source.shape()[2],
                      target.shape()[2]);
        ...
        sd::Tensor<float> output = latent_interposer->compute(n_threads, source);
        if (output.empty()) {
            LOG_ERROR("Latent Interposer compute failed");
            return {};
        }
        if (output.shape() != target.shape()) {
            LOG_ERROR("Latent Interposer output shape %s does not match sampler noise shape %s",
                      sd::tensor_shape_to_string(output.shape()).c_str(),
                      sd::tensor_shape_to_string(target.shape()).c_str());
            return {};
        }
        return output;
    ...
    // Version2 of the user's staged latent generator constructs this
    // deterministic 28-state lattice directly in Flux's 16-channel space.
    sd::Tensor<float> generate_flux_lattice_source(const sd::Tensor<float>& target,
                                                    int phase_x,
                                                    int phase_y) {
        if (target.dim() != 4 || target.empty()) {
            return {};
        }

        const int64_t width  = target.shape()[0];
        const int64_t height = target.shape()[1];
        const int64_t batch  = target.shape()[3];
        sd::Tensor<float> flux_lattice({width, height, 16, batch});

        constexpr float blackness[4] = {5.f / 6.f, 2.f / 6.f, 4.f / 6.f, 3.f / 6.f};
        constexpr float basis4[7][4] = {
            {1.0f, 0.0f, 0.0f, -0.5f},
            {0.8f, 0.8f, 0.0f, -0.3f},
            {0.0f, 0.0f, 1.0f, -0.5f},
            {0.5f, 0.0f, 0.8f, -0.3f},
        ...
        constexpr float gains[4] = {1.0f, 0.75f, -0.55f, 0.35f};
        constexpr int alias[7]   = {2, 2, 2, 3, 2, 2, 6};
        ...
        for (int hue = 0; hue < 7; ++hue) {
            ...
            for (int block = 0; block < 4; ++block) {
                for (int component = 0; component < 4; ++component) {
                    const int channel       = block * 4 + component;
                    basis16[hue][channel]   = basis4[hue][component] * gains[block];
                    norm_sq                += basis16[hue][channel] * basis16[hue][channel];
                }
            }
            const float inv_norm = 1.f / std::max(std::sqrt(norm_sq), 1e-6f);
            for (float& value : basis16[hue]) {
                value *= inv_norm;
            }
        }

        auto positive_mod = [](int64_t value, int mod) -> int {
        ...
        for (int64_t b = 0; b < batch; ++b) {
            for (int64_t y = 0; y < height; ++y) {
                for (int64_t x = 0; x < width; ++x) {
                    const int64_t xx = x + phase_x;
                    const int64_t yy = y + phase_y;
                    const int brightness = positive_mod(xx + 7 * yy, 4);
                    const float magnitude = (1.f - blackness[brightness]) * 1.2f - 0.3f;
                    const int parity       = positive_mod(yy, 2);
                    const int raw_hue      = positive_mod(2 * xx + 4 * yy + parity, 7);
                    const int hue          = alias[raw_hue];
                    for (int channel = 0; channel < 16; ++channel) {
                        flux_lattice.index(x, y, channel, b) = basis16[hue][channel] * magnitude;
                    }
                }
            }
        }
    ...
    sd::Tensor<float> generate_furception_source(const sd::Tensor<float>& target,
                                                  const sd::Tensor<float>& source_image) {
        if (furception_vae == nullptr || source_image.empty()) {
            LOG_ERROR("Furception v1 latent source requires its VAE and deterministic RGB source image");
            return {};
        }
        if (source_image.dim() != 4 || source_image.shape()[2] != 3) {
            LOG_ERROR("Furception source image must be an RGB image tensor");
            return {};
        }

        sd::Tensor<float> image = source_image;
        const std::vector<int64_t> image_shape = {target.shape()[0] * 8,
                                                   target.shape()[1] * 8,
                                                   3,
                                                   target.shape()[3]};
        if (image.shape() != image_shape) {
            image = sd::ops::interpolate(image, image_shape);
        }
        sd::Tensor<float> moments = furception_vae->encode(n_threads,
                                                            image,
                                                            vae_tiling_params,
                                                            circular_x,
                                                            circular_y);
        if (moments.empty() || moments.shape()[2] != 8) {
            LOG_ERROR("Furception VAE did not return an 8-channel posterior tensor");
            return {};
        }
        return sd::ops::chunk(moments, 2, 2)[0] * 0.18215f;
    }

    sd::Tensor<float> generate_interposed_noise(const sd::Tensor<float>& target,
                                                 sd_latent_source_t source_family,
                                                 const sd::Tensor<float>& source_image,
                                                 int phase_x,
                                                 int phase_y) {
        if (source_family <= SD_LATENT_SOURCE_NONE || source_family >= SD_LATENT_SOURCE_COUNT) {
            LOG_ERROR("invalid latent source family: %d", static_cast<int>(source_family));
            return {};
        }

        sd::Tensor<float> source;
        if (source_family == SD_LATENT_SOURCE_V1) {
            source = generate_furception_source(target, source_image);
        } else if (source_family == SD_LATENT_SOURCE_FX) {
            source = generate_flux_lattice_source(target, phase_x, phase_y);
        } else {
            std::vector<int64_t> source_shape = target.shape();
            source_shape[2]                  = latent_source_channels(source_family);
            source                           = sd::Tensor<float>::randn(source_shape, rng);
        }
        return convert_interposed_noise(source, target, source_family);
    }
    ...
    bool select_vae_runtime_backend(SDBackendModule module) {
        if (!first_stage_model) {
            return false;
        }
        ggml_backend_t backend = backend_for(module);
        if (backend == nullptr) {
            return false;
        }
        if (!first_stage_model->set_runtime_backend(backend)) {
            LOG_ERROR("failed to select %s runtime backend for the VAE",
                      sd_backend_module_name(module));
            return false;
        }
        first_stage_model->set_runtime_backends(backend_manager.runtime_backends(module));
        return true;
    }

    sd::Tensor<float> encode_to_vae_latents(const sd::Tensor<float>& x) {
        if (!select_vae_runtime_backend(SDBackendModule::VAE_ENCODE)) {
            return {};
        }
    ...
    sd::Tensor<float> apply_vae_interposer(const std::shared_ptr<LatentInterposerRunner>& runner,
                                           const sd::Tensor<float>& input,
                                           const char* stage) {
        if (!runner) {
            return input;
        }
        if (input.empty() || input.dim() != 4 || input.shape()[2] != runner->in_channels) {
            LOG_ERROR("%s Interposer expected a 4D, %" PRId64 "-channel latent but received %s",
                      stage,
                      runner->in_channels,
                      input.empty() ? "an empty tensor" : sd::tensor_shape_to_string(input.shape()).c_str());
            return {};
        }
        auto output = runner->compute(n_threads, input);
        if (output.empty()) {
            LOG_ERROR("%s Interposer compute failed", stage);
            return {};
        }
        if (output.dim() != 4 || output.shape()[0] != input.shape()[0] || output.shape()[1] != input.shape()[1] ||
            output.shape()[2] != runner->out_channels || output.shape()[3] != input.shape()[3]) {
            LOG_ERROR("%s Interposer produced invalid shape %s from %s",
                      stage,
                      sd::tensor_shape_to_string(output.shape()).c_str(),
                      sd::tensor_shape_to_string(input.shape()).c_str());
            return {};
        }
        return output;
    }

    bool get_model_latent_scale_shift(float& scale, float& shift) const {
        shift = 0.f;
        ...
        if (sd_version_uses_flux_vae(version)) {
            scale = 0.3611f;
            shift = 0.1159f;
            return true;
        }
        LOG_ERROR("VAE Interposer does not support diffusion latent scaling for model version %d",
                  static_cast<int>(version));
        return false;
    }

    sd::Tensor<float> model_vae_to_diffusion_latents(const sd::Tensor<float>& latents) {
        float scale = 1.f;
        float shift = 0.f;
        if (!get_model_latent_scale_shift(scale, shift)) {
            return {};
        }
        return (latents - shift) * scale;
    }

    sd::Tensor<float> model_diffusion_to_vae_latents(const sd::Tensor<float>& latents) {
        float scale = 1.f;
        float shift = 0.f;
        if (!get_model_latent_scale_shift(scale, shift)) {
            return {};
    ...
    sd::Tensor<float> diffusion_to_decode_vae_latents(const sd::Tensor<float>& latents,
                                                       const char* stage) {
        if (!latent_interposer_decode) {
            return first_stage_model->diffusion_to_vae_latents(latents);
        }

        // city96 interposers are trained on the unscaled outputs of each VAE.
        // Undo the checkpoint model's latent format before conversion; the
        // converter output is already raw input for the selected decode VAE.
        auto model_vae_latents = model_diffusion_to_vae_latents(latents);
        if (model_vae_latents.empty()) {
            return {};
        }
        return apply_vae_interposer(latent_interposer_decode, model_vae_latents, stage);
    }
    ...
    sd::Tensor<float> encode_first_stage(const sd::Tensor<float>& x) {
        ...
        if (latent_interposer_encode) {
            // VAE encode returns raw VAE latents, which is the representation
            // used to train the city96 network.  Convert first, then apply the
            // checkpoint model's scale/shift to the converted result.
            latents = apply_vae_interposer(latent_interposer_encode, latents, "Encode");
            if (latents.empty()) {
                return {};
            }
            if (version != VERSION_SD1_PIX2PIX) {
                latents = model_vae_to_diffusion_latents(latents);
            }
    ...
    sd::Tensor<float> decode_first_stage(const sd::Tensor<float>& x, bool decode_video = false) {
        ...
        auto latents = diffusion_to_decode_vae_latents(x, "Decode");
        if (latents.empty()) {
            return {};
        }
        if (!select_vae_runtime_backend(SDBackendModule::VAE_DECODE)) {
            return {};
        }
        auto decoded = first_stage_model->decode(n_threads,
                                                 latents,
                                                 vae_tiling_params,
                                                 decode_video,
                                                 circular_x,
...
const char* sd_vae_format_name(enum sd_vae_format_t format) {
    switch (format) {
        ...
        case SD_VAE_FORMAT_FLUX2:
            return "flux2";
        case SD_VAE_FORMAT_V1:
            return "v1";
        case SD_VAE_FORMAT_XL:
            return "xl";
...
void sd_ctx_params_init(sd_ctx_params_t* sd_ctx_params) {
    *sd_ctx_params                      = {};
    ...
    sd_ctx_params->latent_interposer_path  = nullptr;
    sd_ctx_params->latent_interposer_encode_path = nullptr;
    sd_ctx_params->latent_interposer_decode_path = nullptr;
    sd_ctx_params->furception_vae_path     = nullptr;
}
...
char* sd_ctx_params_to_str(const sd_ctx_params_t* sd_ctx_params) {
    ...
    snprintf(buf + strlen(buf), 8192 - strlen(buf),
             ...
             "taesd_path: %s\n"
             "control_net_path: %s\n"
             "control_net_sd1_path: %s\n"
             "control_net_sdxl_path: %s\n"
             "control_net_lllite_path: %s\n"
             "latent_interposer_path: %s\n"
             "latent_interposer_encode_path: %s\n"
             "latent_interposer_decode_path: %s\n"
             "furception_vae_path: %s\n"
             "photo_maker_path: %s\n"
             ...
             SAFE_STR(sd_ctx_params->ip_adapter_path),
             ...
             SAFE_STR(sd_ctx_params->control_net_sd1_path),
             SAFE_STR(sd_ctx_params->control_net_sdxl_path),
             SAFE_STR(sd_ctx_params->control_net_lllite_path),
             SAFE_STR(sd_ctx_params->latent_interposer_path),
             SAFE_STR(sd_ctx_params->latent_interposer_encode_path),
             SAFE_STR(sd_ctx_params->latent_interposer_decode_path),
             SAFE_STR(sd_ctx_params->furception_vae_path),
             ...
             BOOL_STR(sd_ctx_params->diffusion_sage_attn),
...
void sd_img_gen_params_init(sd_img_gen_params_t* sd_img_gen_params) {
    *sd_img_gen_params = {};
    ...
    sd_img_gen_params->control_net_type                  = CONTROL_NET_TYPE_CANNY;
    sd_img_gen_params->control_net_lllite_strength      = 1.0f;
    sd_img_gen_params->control_net_lllite_start_percent = 0.0f;
    sd_img_gen_params->control_net_lllite_end_percent   = 0.0f;
    sd_img_gen_params->latent_interposer_enabled        = false;
    sd_img_gen_params->latent_interposer_source         = SD_LATENT_SOURCE_NONE;
    sd_img_gen_params->latent_interposer_phase_x        = 0;
    sd_img_gen_params->latent_interposer_phase_y        = 0;
    sd_img_gen_params->ip_adapter_strength                = 1.0f;
    sd_img_gen_params->ip_adapter                         = {{0, 0, 0, nullptr}, 1.0f, 0.0f, 100.0f};
    sd_img_gen_params->qwen_image_layers                  = 3;
    sd_img_gen_params->preserve_cancel_state              = false;
...
char* sd_img_gen_params_to_str(const sd_img_gen_params_t* sd_img_gen_params) {
    ...
    snprintf(buf + strlen(buf), 4096 - strlen(buf),
             ...
             static_cast<int>(sd_img_gen_params->control_net_type),
    ...
    } else if (sd_img_gen_params->cache.mode == SD_CACHE_TEACACHE) {
        cache_mode_str = "teacache";
...
struct GenerationRequest {
    ...
    GenerationRequest(sd_ctx_t* sd_ctx, const sd_img_gen_params_t* sd_img_gen_params) {
        ...
        control_type                = static_cast<int>(sd_img_gen_params->control_net_type);
        control_net_lllite_strength      = sd_img_gen_params->control_net_lllite_strength;
        control_net_lllite_start_percent = sd_img_gen_params->control_net_lllite_start_percent;
        control_net_lllite_end_percent   = sd_img_gen_params->control_net_lllite_end_percent;
        latent_interposer_enabled        = sd_img_gen_params->latent_interposer_enabled;
        latent_interposer_source         = sd_img_gen_params->latent_interposer_source;
        latent_interposer_phase_x        = sd_img_gen_params->latent_interposer_phase_x;
        latent_interposer_phase_y        = sd_img_gen_params->latent_interposer_phase_y;
        ...
        const bool has_scheduled_ip_adapter = sd_img_gen_params->ip_adapter.image.data != nullptr;
        ip_adapter_strength                 = has_scheduled_ip_adapter
                                                  ? sd_img_gen_params->ip_adapter.strength
                                                  : (sd_img_gen_params->ip_adapter_image.data != nullptr
                                                         ? sd_img_gen_params->ip_adapter_strength
                                                         : 0.f);
        ip_adapter_start_percent = has_scheduled_ip_adapter
                                       ? std::clamp(sd_img_gen_params->ip_adapter.start_percent, 0.f, 100.f)
                                       : 0.f;
        ip_adapter_end_percent   = has_scheduled_ip_adapter
                                       ? std::clamp(sd_img_gen_params->ip_adapter.end_percent, 0.f, 100.f)
                                       : 100.f;
...
struct ImageGenerationLatents {
    ...
    sd::Tensor<float> control_net_lllite_image;
    sd::Tensor<float> latent_interposer_source_image;
...
static std::optional<ImageGenerationLatents> prepare_image_generation_latents(sd_ctx_t* sd_ctx,
    ...
    sd::Tensor<float> control_net_lllite_image_tensor;
    sd::Tensor<float> latent_interposer_source_image_tensor;
    ...
    if (sd_img_gen_params->control_net_lllite_image.data != nullptr) {
        control_net_lllite_image_tensor = sd_image_to_tensor(sd_img_gen_params->control_net_lllite_image,
                                                              request->width,
                                                              request->height);
        control_net_lllite_image_tensor = control_net_lllite_image_tensor * 2.f - 1.f;
    }

    if (sd_img_gen_params->latent_interposer_source_image.data != nullptr) {
        latent_interposer_source_image_tensor = sd_image_to_tensor(sd_img_gen_params->latent_interposer_source_image,
                                                                   request->width,
                                                                   request->height);
    }
    ...
    latents.control_net_lllite_image = std::move(control_net_lllite_image_tensor);
    latents.latent_interposer_source_image = std::move(latent_interposer_source_image_tensor);
...
static std::optional<ImageGenerationEmbeds> prepare_image_generation_embeds(sd_ctx_t* sd_ctx,
    ...
    const sd_image_t& ip_adapter_image = sd_img_gen_params->ip_adapter.image.data != nullptr
    ...
    if (ip_adapter_image.data != nullptr && request->ip_adapter_strength != 0.f) {
        ...
        if (ip_adapter_image.channel != 3) {
            LOG_ERROR("IP-Adapter input must be an RGB image");
            return std::nullopt;
        }
        if (request->ip_adapter_end_percent <= request->ip_adapter_start_percent) {
            LOG_ERROR("IP-Adapter end_percent must be greater than start_percent");
            return std::nullopt;
        }
    }
    if (!sd_ctx->sd->compute_ip_adapter_tokens(ip_adapter_image, request->ip_adapter_strength)) {
        LOG_ERROR("IP-Adapter conditioning could not be prepared; generation cancelled without terminating the backend");
        return std::nullopt;
    }
    int64_t prepare_start_ms         = ggml_time_ms();
    ...
    if (sd_ctx->sd->uni_control_global_adapter != nullptr &&
        sd_img_gen_params->control_strength > 0.f) {
        if (latents->control_image.empty() || sd_ctx->sd->clip_vision == nullptr) {
            LOG_ERROR("Uni-ControlNet global control requires a content image and OpenAI CLIP ViT-L/14");
            return std::nullopt;
        }

        auto clip_embedding = sd_ctx->sd->get_clip_vision_output(latents->control_image, true, -1);
        if (clip_embedding.empty()) {
            LOG_ERROR("failed to compute Uni-ControlNet global CLIP embedding");
            return std::nullopt;
        }
        auto global_tokens = sd_ctx->sd->uni_control_global_adapter->compute(sd_ctx->sd->n_threads,
                                                                             clip_embedding);
        if (global_tokens.empty()) {
            LOG_ERROR("failed to compute Uni-ControlNet global content tokens");
            return std::nullopt;
        }
        global_tokens = global_tokens * sd_img_gen_params->control_strength;
        cond.c_crossattn = sd::ops::concat(cond.c_crossattn, global_tokens, 1);

        if (!uncond.c_crossattn.empty() || !img_uncond.c_crossattn.empty()) {
            auto zero_embedding = sd::Tensor<float>::zeros_like(clip_embedding);
            auto zero_tokens = sd_ctx->sd->uni_control_global_adapter->compute(sd_ctx->sd->n_threads,
                                                                               zero_embedding);
            if (zero_tokens.empty()) {
                LOG_ERROR("failed to compute unconditional Uni-ControlNet global content tokens");
                return std::nullopt;
            }
            zero_tokens = zero_tokens * sd_img_gen_params->control_strength;
            if (!uncond.c_crossattn.empty()) {
                uncond.c_crossattn = sd::ops::concat(uncond.c_crossattn, zero_tokens, 1);
            }
            if (!img_uncond.c_crossattn.empty()) {
...
SD_API bool generate_image(sd_ctx_t* sd_ctx,
    ...
    if (!sd_img_gen_params->preserve_cancel_state) {
    ...
    if (request.latent_interposer_enabled) {
        if (request.latent_interposer_source <= SD_LATENT_SOURCE_NONE ||
            request.latent_interposer_source >= SD_LATENT_SOURCE_COUNT) {
            LOG_ERROR("Latent Interposer noise was requested without a valid source family");
            return false;
        }
        LOG_INFO("using staged Latent Interposer source %d (phase %d,%d)",
                 static_cast<int>(request.latent_interposer_source),
                 request.latent_interposer_phase_x,
                 request.latent_interposer_phase_y);
    }
    ...
    for (int b = 0; b < request.batch_count; b++) {
        ...
        sd::Tensor<float> noise;
        if (request.latent_interposer_enabled) {
            noise = sd_ctx->sd->generate_interposed_noise(latents.init_latent,
                                                           request.latent_interposer_source,
                                                           latents.latent_interposer_source_image,
                                                           request.latent_interposer_phase_x,
                                                           request.latent_interposer_phase_y);
            if (noise.empty()) {
                return false;
            ...
            noise = sd::randn_like<float>(latents.init_latent, sd_ctx->sd->rng);
        }
        ...
        sd::Tensor<float> x_0 = sd_ctx->sd->sample(sd_ctx->sd->diffusion_model,
                                                   ...
                                                   true,
                                                   sd::Tensor<float>(),
                                                   latents.control_net_lllite_image,
                                                   request.control_net_lllite_strength,
                                                   request.control_net_lllite_start_percent,
                                                   request.control_net_lllite_end_percent,
                                                   request.control_type,
                                                   request.ip_adapter_strength,
                                                   request.ip_adapter_start_percent,
                                                   request.ip_adapter_end_percent);
    ...
    if (request.hires.enabled && request.hires.target_width > 0) {
        ...
        for (int b = 0; b < (int)final_latents.size(); b++) {
            ...
            sd::Tensor<float> noise;
            if (request.latent_interposer_enabled) {
                noise = sd_ctx->sd->generate_interposed_noise(upscaled,
                                                               request.latent_interposer_source,
                                                               latents.latent_interposer_source_image,
                                                               request.latent_interposer_phase_x,
                                                               request.latent_interposer_phase_y);
                ...
                noise = sd::randn_like<float>(upscaled, sd_ctx->sd->rng);
            }
            ...
            sd::Tensor<float> hires_lllite_image;
            if (!latents.control_net_lllite_image.empty()) {
                hires_lllite_image = sd::ops::interpolate(latents.control_net_lllite_image,
                                                           {upscaled.shape()[0] * request.vae_scale_factor,
                                                            upscaled.shape()[1] * request.vae_scale_factor,
                                                            latents.control_net_lllite_image.shape()[2],
                                                            latents.control_net_lllite_image.shape()[3]});
            }
            sd::Tensor<float> x_0      = sd_ctx->sd->sample(sd_ctx->sd->diffusion_model,
                                                            true,
                                                            upscaled,
...
static std::optional<ImageGenerationLatents> prepare_video_generation_latents(sd_ctx_t* sd_ctx,
    ...
    } else if (sd_ctx->sd->version == VERSION_SVD) {
...
static bool generate_animatediff_video(sd_ctx_t* sd_ctx,
    ...
    img_gen_params.preserve_cancel_state = sd_vid_gen_params->preserve_cancel_state;
