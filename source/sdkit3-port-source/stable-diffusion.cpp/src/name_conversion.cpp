...
std::string convert_controlnet_tensor_name(std::string name, SDVersion version) {
    // PyTorch ControlNet checkpoints commonly wrap the original CompVis names in
    // "control_model.". Diffusers checkpoints instead store the shared UNet
    // blocks using Diffusers names and give the ControlNet-only blocks their own
    // prefixes. Normalize both layouts to the names used by ControlNetBlock.
    if (starts_with(name, "control_model.")) {
        name = name.substr(strlen("control_model."));
    }
    if (starts_with(name, "local_adapter.")) {
        name = name.substr(strlen("local_adapter."));
    }

    // torch.nn.MultiheadAttention uses the flat in_proj_weight/in_proj_bias
    // spelling. GGMLBlock represents that projection as a Linear child.
    static const std::string union_attn_prefix = "transformer_layes.0.attn.";
    if (starts_with(name, union_attn_prefix + "in_proj_weight")) {
        return union_attn_prefix + "in_proj.weight";
    }
    if (starts_with(name, union_attn_prefix + "in_proj_bias")) {
        return union_attn_prefix + "in_proj.bias";
    }

    static const std::string cond_embedding_prefix = "controlnet_cond_embedding.";
    if (starts_with(name, cond_embedding_prefix)) {
        std::string suffix = name.substr(cond_embedding_prefix.size());
        if (starts_with(suffix, "conv_in.")) {
            return "input_hint_block.0." + suffix.substr(strlen("conv_in."));
        }
        if (starts_with(suffix, "conv_out.")) {
            return "input_hint_block.14." + suffix.substr(strlen("conv_out."));
        }
        if (starts_with(suffix, "blocks.")) {
            const size_t index_start = strlen("blocks.");
            const size_t index_end   = suffix.find('.', index_start);
            if (index_end != std::string::npos) {
                try {
                    const int block_index = std::stoi(suffix.substr(index_start, index_end - index_start));
                    return "input_hint_block." + std::to_string(2 + 2 * block_index) + suffix.substr(index_end);
                } catch (const std::exception&) {
                    // Preserve an unknown name so model validation reports it.
                }
            }
        }
    }

    static const std::string down_blocks_prefix = "controlnet_down_blocks.";
    if (starts_with(name, down_blocks_prefix)) {
        const size_t index_start = down_blocks_prefix.size();
        const size_t index_end   = name.find('.', index_start);
        if (index_end != std::string::npos) {
            try {
                const int block_index = std::stoi(name.substr(index_start, index_end - index_start));
                return "zero_convs." + std::to_string(block_index) + ".0" + name.substr(index_end);
            } catch (const std::exception&) {
                // Preserve an unknown name so model validation reports it.
            }
        }
    ...
    static const std::string middle_block_prefix = "controlnet_mid_block.";
    if (starts_with(name, middle_block_prefix)) {
        return "middle_block_out.0." + name.substr(middle_block_prefix.size());
    }

    return convert_diffusion_model_name(std::move(name), "", version);
}
