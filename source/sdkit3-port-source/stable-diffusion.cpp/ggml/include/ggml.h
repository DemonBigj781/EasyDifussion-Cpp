...
extern "C" {
    ...
    // Prefer the native SageAttention CUDA kernel when this operation is
    // compatible. Backends must fall back to their regular implementation.
    GGML_API void ggml_flash_attn_ext_set_sage_attn(
            struct ggml_tensor * a,
            bool                 enabled);

    GGML_API bool ggml_flash_attn_ext_get_sage_attn(
            const struct ggml_tensor * a);
