...
#include "ggml-cuda/flash-attention.cuh"
...
static bool ggml_cuda_flash_common_requested() {
    const char * env = std::getenv("SD_CUDA_FLASH_COMMON");
    return env != nullptr && env[0] == '1';
}
...
#if defined(GGML_USE_VMM)
    const bool force_cuda_malloc = getenv("GGML_CUDA_FORCE_MALLOC") != nullptr;
    if (ggml_cuda_info().devices[device].vmm && !force_cuda_malloc) {
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            if (ggml_cuda_flash_common_requested() &&
                ggml_cuda_flash_common_attn_supported(ctx.device, dst)) {
                ggml_cuda_flash_common_attn(ctx, dst);
            } else if (ggml_cuda_xformers_requested() &&
    ...
    switch (op->op) {
        ...
        case GGML_OP_DSV4_HC_POST:
            return op->src[0]->type == GGML_TYPE_F32 && op->src[1]->type == GGML_TYPE_F32 &&
                ...
                op->type == GGML_TYPE_F32;
        case GGML_OP_FLASH_ATTN_EXT:
            return (ggml_cuda_flash_common_requested() &&
                    ggml_cuda_flash_common_attn_supported(dev_ctx->device, op)) ||
                   (ggml_cuda_xformers_requested() &&
