...
// The original PyTorch launch wrappers are retained below for source traceability,
// but excluded from this native ggml build. The raw launcher after this block uses
// ggml tensors and its CUDA stream directly.
#if 0
...
template <typename T>
__device__ __forceinline__ float sage_to_float(T value);

template <>
__device__ __forceinline__ float sage_to_float<float>(float value) {
    return value;
}

template <>
__device__ __forceinline__ float sage_to_float<half>(half value) {
    return __half2float(value);
}

template <typename T, int HEAD_DIM, int BLOCK_TOKENS, bool SUBTRACT_MEAN>
__global__ void sage_quantize_int8(
        ...
        uint32_t num_tokens,
        uint32_t num_heads,
        uint32_t num_quant_blocks,
        uint64_t stride_batch,
        uint64_t stride_token,
        uint64_t stride_head) {
    const uint32_t quant_block = blockIdx.x;
    const uint32_t head = blockIdx.y;
    const uint32_t batch = blockIdx.z;
    const uint32_t token_begin = quant_block * BLOCK_TOKENS;

    float local_amax = 1.0e-7f;
    for (uint32_t linear = threadIdx.x; linear < BLOCK_TOKENS * HEAD_DIM; linear += blockDim.x) {
        const uint32_t token_offset = linear / HEAD_DIM;
        const uint32_t dim = linear % HEAD_DIM;
        const uint32_t token = token_begin + token_offset;
        if (token < num_tokens) {
            const uint64_t input_index = uint64_t(batch) * stride_batch +
                                         uint64_t(token) * stride_token +
                                         uint64_t(head) * stride_head + dim;
            float value = sage_to_float(input[input_index]);
            if constexpr (SUBTRACT_MEAN) {
                value -= mean[(uint64_t(batch) * num_heads + head) * HEAD_DIM + dim];
            }
    ...
    if ((threadIdx.x & 31) == 0) {
        warp_max[threadIdx.x >> 5] = local_amax;
    }
    __syncthreads();

    float block_amax = threadIdx.x < 8 ? warp_max[threadIdx.x] : 1.0e-7f;
    if (threadIdx.x < 32) {
        for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    if (threadIdx.x == 0) {
        warp_max[0] = block_amax;
        scales[(uint64_t(batch) * num_heads + head) * num_quant_blocks + quant_block] = block_amax / 127.0f;
    }
    __syncthreads();

    const float inv_scale = 127.0f / warp_max[0];
    for (uint32_t linear = threadIdx.x; linear < BLOCK_TOKENS * HEAD_DIM; linear += blockDim.x) {
        const uint32_t token_offset = linear / HEAD_DIM;
        const uint32_t dim = linear % HEAD_DIM;
        const uint32_t token = token_begin + token_offset;
        if (token < num_tokens) {
            ...
            value *= inv_scale;
            const int quantized = max(-127, min(127, __float2int_rn(value)));
            const uint64_t output_index = ((uint64_t(batch) * num_tokens + token) * num_heads + head) * HEAD_DIM + dim;
            output[output_index] = static_cast<int8_t>(quantized);
        }
    }
}

template <int HEAD_DIM>
__global__ void sage_key_mean(
    ...
    for (uint32_t token = 0; token < num_tokens; ++token) {
        const uint64_t index = uint64_t(batch) * stride_batch +
                               uint64_t(token) * stride_token +
                               uint64_t(head) * stride_head + dim;
        sum += __half2float(key[index]);
    }
    mean[(uint64_t(batch) * num_heads + head) * HEAD_DIM + dim] = sum / num_tokens;
}

__global__ void sage_half_to_float(const half * src, float * dst, uint64_t nelements) {
...
template <int HEAD_DIM>
void launch_sage_attention_sm80(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const uint32_t qo_len = Q->ne[1];
    const uint32_t kv_len = K->ne[1];
    const uint32_t num_qo_heads = Q->ne[2];
    const uint32_t num_kv_heads = K->ne[2];
    const uint32_t batch_size = Q->ne[3];
    const uint32_t num_kv_groups = num_qo_heads / num_kv_heads;
    const uint32_t q_scale_blocks = div_ceil(qo_len, CTA_Q) * (CTA_Q / WARP_Q);
    const uint32_t k_scale_blocks = div_ceil(kv_len, CTA_K);

    ggml_cuda_pool & pool = ctx.pool();
    ggml_cuda_pool_alloc<int8_t> q_int8(pool, ggml_nelements(Q));
    ggml_cuda_pool_alloc<int8_t> k_int8(pool, ggml_nelements(K));
    ggml_cuda_pool_alloc<float> q_scale(pool, uint64_t(batch_size) * num_qo_heads * q_scale_blocks);
    ggml_cuda_pool_alloc<float> k_scale(pool, uint64_t(batch_size) * num_kv_heads * k_scale_blocks);
    ggml_cuda_pool_alloc<float> k_mean(pool, uint64_t(batch_size) * num_kv_heads * HEAD_DIM);
    ggml_cuda_pool_alloc<half> output_f16(pool, ggml_nelements(dst));

    cudaStream_t stream = ctx.stream();
    const dim3 quant_threads(256, 1, 1);
    const dim3 q_quant_grid(q_scale_blocks, num_qo_heads, batch_size);
    const dim3 k_quant_grid(k_scale_blocks, num_kv_heads, batch_size);

    sage_key_mean<HEAD_DIM><<<dim3(num_kv_heads, batch_size, 1), HEAD_DIM, 0, stream>>>(
        static_cast<const half *>(K->data), k_mean.ptr, kv_len, num_kv_heads,
        K->nb[3] / sizeof(half), K->nb[1] / sizeof(half), K->nb[2] / sizeof(half));
    sage_quantize_int8<float, HEAD_DIM, WARP_Q, false><<<q_quant_grid, quant_threads, 0, stream>>>(
        static_cast<const float *>(Q->data), nullptr, q_int8.ptr, q_scale.ptr,
        qo_len, num_qo_heads, q_scale_blocks,
        Q->nb[3] / sizeof(float), Q->nb[1] / sizeof(float), Q->nb[2] / sizeof(float));
    sage_quantize_int8<half, HEAD_DIM, CTA_K, true><<<k_quant_grid, quant_threads, 0, stream>>>(
        static_cast<const half *>(K->data), k_mean.ptr, k_int8.ptr, k_scale.ptr,
        kv_len, num_kv_heads, k_scale_blocks,
        K->nb[3] / sizeof(half), K->nb[1] / sizeof(half), K->nb[2] / sizeof(half));
    CUDA_CHECK(cudaGetLastError());

    float sm_scale;
    memcpy(&sm_scale, dst->op_params, sizeof(sm_scale));
    ...
    using sage_kernel_t = decltype(&qk_int_sv_f16_attn_kernel<
    ...
    sage_kernel_t kernel = qk_int_sv_f16_attn_kernel<
        CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM,
        DataType::kInt8, QuantGranularity::kPerWarp, QuantGranularity::kPerWarp,
        float, false, half, ComputeUnit::kTensorCore, MaskMode::kNone, false, false>;
    ...
    const dim3 attention_grid(div_ceil(qo_len, CTA_Q), num_qo_heads, batch_size);
    const dim3 attention_threads(32, (CTA_Q / WARP_Q) * (CTA_K / WARP_K), 1);
    kernel<<<attention_grid, attention_threads, smem_max, stream>>>(
        q_int8.ptr, k_int8.ptr, static_cast<half *>(V->data), output_f16.ptr, nullptr,
        q_scale.ptr, k_scale.ptr, nullptr,
        qo_len, kv_len, num_kv_groups,
        qo_len * num_qo_heads * HEAD_DIM, num_qo_heads * HEAD_DIM, HEAD_DIM,
        kv_len * num_kv_heads * HEAD_DIM, num_kv_heads * HEAD_DIM, HEAD_DIM,
        V->nb[3] / sizeof(half), V->nb[1] / sizeof(half), V->nb[2] / sizeof(half),
        qo_len * num_qo_heads * HEAD_DIM, num_qo_heads * HEAD_DIM, HEAD_DIM,
        sm_scale);
    CUDA_CHECK(cudaGetLastError());

    constexpr uint32_t convert_threads = 256;
    const uint64_t output_elements = ggml_nelements(dst);
    const uint32_t convert_blocks = (output_elements + convert_threads - 1) / convert_threads;
    sage_half_to_float<<<convert_blocks, convert_threads, 0, stream>>>(
        output_f16.ptr, static_cast<float *>(dst->data), output_elements);
    CUDA_CHECK(cudaGetLastError());
}
...
bool ggml_cuda_sage_attn_sm80_supported(int device, const ggml_tensor * dst) {
    ...
    if (Q == nullptr || K == nullptr || V == nullptr || mask != nullptr || sinks != nullptr) {
        return false;
    }

    const int cc = ggml_cuda_info().devices[device].cc;
    if (cc < GGML_CUDA_CC_AMPERE || cc >= GGML_CUDA_CC_HOPPER) {
        return false;
    }
    if (Q->type != GGML_TYPE_F32 || K->type != GGML_TYPE_F16 || V->type != GGML_TYPE_F16 || dst->type != GGML_TYPE_F32) {
        return false;
    }
    if ((Q->ne[0] != 64 && Q->ne[0] != 128) || K->ne[0] != Q->ne[0] || V->ne[0] != Q->ne[0]) {
        return false;
    }
    if (Q->ne[1] <= 0 || K->ne[1] <= 0 || Q->ne[2] <= 0 || K->ne[2] <= 0 || Q->ne[2] % K->ne[2] != 0) {
        return false;
    }
    if (Q->ne[3] != K->ne[3] || Q->ne[3] != V->ne[3]) {
        return false;
    }
    if (Q->nb[0] != sizeof(float) || K->nb[0] != sizeof(half) || V->nb[0] != sizeof(half) || !ggml_is_contiguous(dst)) {
        return false;
    }
...
void ggml_cuda_sage_attn_sm80(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    GGML_ASSERT(ggml_cuda_sage_attn_sm80_supported(ctx.device, dst));
    switch (dst->src[0]->ne[0]) {
        case 64:
            launch_sage_attention_sm80<64>(ctx, dst);
            return;
        case 128:
            launch_sage_attention_sm80<128>(ctx, dst);
            return;
        default:
            GGML_ABORT("unsupported SageAttention head dimension");
    }
}
