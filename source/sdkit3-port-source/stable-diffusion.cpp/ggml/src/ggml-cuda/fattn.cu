...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    // --xformers is process-global in the native server. Prefer its complete
    // GGML_OP_FLASH_ATTN_EXT forward path when requested, then fall through to
    // Sage/ggml FlashAttention whenever the tensor contract is unsupported.
