...
// Semantic ControlNet condition categories. Architecture-specific adapters
// map these to their learned slot ordering; ordinary checkpoints ignore it.
enum control_net_type_t {
    CONTROL_NET_TYPE_CANNY = 0,
    CONTROL_NET_TYPE_MLSD,
    CONTROL_NET_TYPE_HED,
    CONTROL_NET_TYPE_SKETCH,
    CONTROL_NET_TYPE_OPENPOSE,
    CONTROL_NET_TYPE_DEPTH,
    CONTROL_NET_TYPE_NORMAL,
    CONTROL_NET_TYPE_SEGMENT,
    CONTROL_NET_TYPE_GLOBAL,
    CONTROL_NET_TYPE_COUNT,
};
...
// Source latent families supported by the city96 v4.0 interposer matrix.
// Stable Cascade is deliberately excluded until the host backend supports it.
enum sd_latent_source_t {
    SD_LATENT_SOURCE_NONE = 0,
    SD_LATENT_SOURCE_V1,
    SD_LATENT_SOURCE_XL,
    SD_LATENT_SOURCE_V3,
    SD_LATENT_SOURCE_FX,
    SD_LATENT_SOURCE_COUNT,
};
...
typedef struct {
    ...
    // When both are set, select the ControlNet after detecting the base model
    // family from its weights. SD1.x uses Uni-ControlNet and SDXL uses Union.
    const char* control_net_sd1_path;
    const char* control_net_sdxl_path;
    const char* control_net_lllite_path;
    const char* latent_interposer_path;
    const char* latent_interposer_encode_path;
    const char* latent_interposer_decode_path;
    const char* furception_vae_path;
...
typedef struct {
    ...
    int original_width;
    int original_height;
} sd_flex_attention_params_t;

// Select high-resolution visual tokens using the UMass FlexAttention
// low-resolution attention saliency policy. Input is laid out as
// [head_count, query_count, low_grid_size * low_grid_size].
SD_API size_t sd_flex_attention_mask_size(int query_count, int high_grid_size);
SD_API bool sd_flex_attention_select(const float* attention,
                                     int head_count,
                                     int query_count,
...
typedef struct {
    ...
    enum control_net_type_t control_net_type;
    sd_image_t control_net_lllite_image;
    float control_net_lllite_strength;
    float control_net_lllite_start_percent;
    float control_net_lllite_end_percent;
    bool latent_interposer_enabled;
    enum sd_latent_source_t latent_interposer_source;
    sd_image_t latent_interposer_source_image;
    int latent_interposer_phase_x;
    int latent_interposer_phase_y;
    sd_image_t ip_adapter_image;
    float ip_adapter_strength;
    sd_pm_params_t pm_params;
    sd_pulid_params_t pulid_params;
    sd_ip_adapter_params_t ip_adapter;
...
// Sampling-only progress. Unlike the legacy progress callback, this is not
// invoked by model loading, tensor transfers, text encoding, or VAE tiling.
SD_API void sd_set_sample_progress_callback(sd_progress_cb_t cb, void* data);
