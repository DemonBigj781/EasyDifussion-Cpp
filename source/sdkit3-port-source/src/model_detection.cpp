...
bool isFullCheckpoint(const std::string& model_path) {
    const std::string format = detectModelFormat(model_path);
    if (format == "unknown") {
        return false;
    }

    const auto tensor_keys = extractTensorKeys(model_path, format);
    bool has_diffusion     = false;
    bool has_conditioner   = false;
    bool has_vae           = false;
    for (const auto& raw_name : tensor_keys) {
        std::string name = raw_name;
        std::transform(name.begin(), name.end(), name.begin(), ::tolower);
        has_diffusion = has_diffusion ||
                        name.find("model.diffusion_model.") != std::string::npos ||
                        name.find("diffusion_model.") == 0;
        has_conditioner = has_conditioner ||
                          name.find("cond_stage_model.") != std::string::npos ||
                          name.find("conditioner.embedders.") != std::string::npos ||
                          name.find("text_encoders.") == 0;
        has_vae = has_vae ||
                  name.find("first_stage_model.") != std::string::npos ||
                  name.find("vae.") == 0;
    }
    return has_diffusion && (has_conditioner || has_vae);
}
