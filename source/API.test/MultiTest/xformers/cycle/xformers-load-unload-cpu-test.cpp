...
int main(int argc, char** argv) {
    ...
    easyapi::CpuModelLifecycleHandler lifecycle;
    ok &= check(std::string_view(lifecycle.name()) == "cpu",
                "CPU model lifecycle handler is available");
    ...
    bool cycle_ok = true;
    const int cycle_count = fixture.bytes.size() > (64u * 1024u * 1024u) ? 1 : 32;
    for (int cycle = 0; cycle < cycle_count; ++cycle) {
        auto loaded = lifecycle.load(fixture.bytes.data(), fixture.bytes.size());
        cycle_ok &= loaded.success && loaded.resource.loaded();
        cycle_ok &= loaded.resource.storage == easyapi::ModelStorage::cpu;
        ...
        std::vector<float> cycle_q{1.0f, 0.0f, 0.0f, 1.0f};
        std::vector<float> cycle_k{1.0f, 0.0f, 0.0f, 1.0f};
        std::vector<float> cycle_v{10.0f, 20.0f, 30.0f, 40.0f};
        std::vector<float> cycle_out(4);
        AttentionRequest cycle_request = make_basic_request(
            cycle_q.data(), cycle_k.data(), cycle_v.data(), cycle_out.data());
        cycle_ok &= forward(Backend::cpu, cycle_request);
        cycle_ok &= nearly_equal(cycle_out[0], expected[0]);
        cycle_ok &= nearly_equal(cycle_out[3], expected[3]);
        if (loaded.success) {
    ...
    ok &= check(cycle_ok,
                "CPU xFormers survives Common model load/forward/unload cycles");
    std::cout << "CPU lifecycle cycles: " << cycle_count
              << " using " << fixture.path << '\n';
