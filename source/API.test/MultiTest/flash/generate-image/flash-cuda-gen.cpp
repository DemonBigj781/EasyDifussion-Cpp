...
// Observation hook from GGML's application adapter. Image execution still
// enters through normal stable-diffusion.cpp graph dispatch.
std::uint64_t ggml_cuda_flash_common_attn_launch_count() noexcept;
...
bool enable_flash_common() {
#ifdef _WIN32
    return _putenv_s("SD_CUDA_XFORMERS", "0") == 0 &&
           _putenv_s("SD_CUDA_FLASH_COMMON", "1") == 0;
#else
    return setenv("SD_CUDA_XFORMERS", "0", 1) == 0 &&
           setenv("SD_CUDA_FLASH_COMMON", "1", 1) == 0;
#endif
}

std::uint64_t flash_launch_count() {
    return ggml_cuda_flash_common_attn_launch_count();
}
...
int main(int argc, char** argv) {
    const api_test::attention_image::BackendConfiguration backend{
        "CUDA",
        "0",
        "flash-cuda-output.ppm",
        "normalized FlashAttention Common",
        &enable_flash_common,
        &flash_launch_count,
        &resolve_cuda,
    };
