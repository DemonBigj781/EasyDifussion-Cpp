...
`source/DiffUser.cpp` is the canonical project tree. `source/API.cpp` is retained
only as a compatibility link to this directory.

`source/DiffUser.cpp` normalizes backend-specific compute behavior behind one Common interface for Easy Diffusion.
...
- `cmake/` and `Makefile` remain at the DiffUser.cpp root.
