...
A future source/CI audit should fail when a file under `source/DiffUser.cpp/library/` includes a path containing `/translation/` or `/definition/`. Equivalent direct backend dependencies should also be rejected during review even if expressed through an alias or wrapper.
