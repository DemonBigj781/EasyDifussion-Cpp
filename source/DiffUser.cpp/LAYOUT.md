# DiffUser.cpp architecture

## Purpose

`source/DiffUser.cpp` is the backend-normalization and Common library boundary
...
source/DiffUser.cpp/
├── Makefile
├── cmake/                         # root DiffUser.cpp build helpers
...
`cmake/` and `Makefile` stay directly under `source/DiffUser.cpp`. Backend implementation directories, public include trees, and Library sources must not be reintroduced at the DiffUser.cpp root.
...
- CUDA attention implementation ownership moved from `DiffUser.cpp/cuda/attention/*` to each feature's `cuda/translation/gpu`.
- The Theory-only `source/API.bridge/sdkit3-ggml` xFormers and Flash adapters
  ...
  their DiffUser.cpp ownership explicit.
