...
```text
DiffUser.cpp/features/attention/split/
├── common/
│   ├── plan.cpp
│   ├── split.cpp
│   ├── qkt.cpp
...
Each definition branch has a parallel
`[backend]/translation/[device-class]/[method].cpp` branch as specified by
`DiffUser.cpp/LAYOUT.md`. Definitions capture native behavior; translations adapt it to Common.

## Core rule

Translate first. Unify second.

A backend/device definition answers: "What native behavior and constraints does this target expose for the normalized Split Attention operation?"

A backend/device translation answers: "How is that native definition adapted into the normalized Split Attention contract, including required chunking, slicing, temporary storage, synchronization, or workaround?"
...
5. Capture each method's native behavior in backend/device definitions.
6. Implement the parallel backend-to-common translations.
7. Document how each target chooses its split plan and preserves global softmax semantics.
8. Compare translations across targets and identify genuinely shared behavior.
9. Design the Common contract from those translations.
10. Implement `common/[method].cpp` only after translation coverage is sufficiently complete.
11. Route the Library to Common only after Common is stable.
