...
std::size_t mask_index(const MaskView & m,
                       std::int64_t b,
                       std::int64_t h,
                       std::int64_t q,
                       std::int64_t k) {
...
bool apply_mask_and_bias(const AttentionRequest & request, ScoreBuffer & scores) {
    ...
    for (std::int64_t b = 0; b < scores.batch; ++b) {
        for (std::int64_t h = 0; h < scores.heads; ++h) {
            const float alibi_slope = request.alibi.enabled && request.alibi.slopes &&
                                      h < request.alibi.slope_count
                ? request.alibi.slopes[h]
                : 0.0f;
            const float sink_bias = request.sinks.enabled && request.sinks.values &&
                                    h < request.sinks.value_count
                ? request.sinks.values[h]
                : 0.0f;
