...
Result detect() {
    ...
    for (auto& native_device : native.devices) {
        ...
        device.architecture = "sm_" + std::to_string(device.compute_major) + std::to_string(device.compute_minor);
...
namespace {
const bool registered = edcpp::api::detect::register_translation(Backend::cuda, &detect);
}
