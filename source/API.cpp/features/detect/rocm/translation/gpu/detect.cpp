...
#include "features/detect/rocm/definition/gpu/detect.hpp"
...
Result detect() {
    ...
    for (auto& native_device : native.devices) {
        ...
        if (device.architecture.empty()) {
            device.architecture = "compute_" + std::to_string(native_device.compute_major) +
                                  std::to_string(native_device.compute_minor);
        }
