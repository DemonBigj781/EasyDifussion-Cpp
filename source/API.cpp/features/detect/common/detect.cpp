...
Result normalize(Result result) {
    if (result.backend == Backend::none) {
        ...
        if (result.diagnostic.empty()) {
            result.diagnostic = "detect translation did not identify a backend";
        }
        return result;
    }

    if (result.backend_name.empty()) {
        result.backend_name = edcpp::api::backend_name(result.backend);
    }
    ...
    const bool have_available_device = std::any_of(
        result.devices.begin(), result.devices.end(),
        [](const Device& device) { return device.available; });
    result.backend_available = result.backend_available && have_available_device;

    if (!result.backend_available && result.diagnostic.empty()) {
        result.diagnostic = "backend reported no available devices";
    }
