...
// Runtime-test observation point. The count changes only when the native
// xFormers implementation is selected and launched, so an image test can
// distinguish that path from the normal ggml FlashAttention fallback.
