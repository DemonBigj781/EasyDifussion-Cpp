...
PERCHANCE = Path(os.environ.get("PERCHANCE_LAUNCHER", "/home/jack/bin/perchance")).expanduser()
PERCHANCE_LOCK = threading.Lock()
SETTINGS_LOCK = threading.Lock()
IMAGE_TIMEOUT_SECONDS = 15 * 60
TEXT_TIMEOUT_SECONDS = 10 * 60
GALLERY_TIMEOUT_SECONDS = 15 * 60
...
def save_settings(payload) -> dict:
    ...
    with SETTINGS_LOCK:
        ...
        config["perchance"] = {"gallery_id": gallery_id, "channel": channel}
...
async def _invoke(arguments: list[str], timeout_seconds: int, environment=None) -> dict:
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Could not start {PERCHANCE}: {error}",
        ) from error
...
async def _run_perchance(arguments: list[str], timeout_seconds: int) -> dict:
    if not PERCHANCE.is_file() or not os.access(PERCHANCE, os.X_OK):
        raise HTTPException(
            status_code=500,
            detail=f"Perchance launcher is missing or not executable: {PERCHANCE}",
        )

    result = await _invoke(arguments, timeout_seconds)
    ...
    if result["returncode"] != 0 and any(
        ...
        result = await _invoke(arguments, timeout_seconds, environment)
...
def status() -> dict:
    try:
        resolved = str(PERCHANCE.resolve(strict=True))
    except OSError:
        resolved = ""
    return {
        "ready": PERCHANCE.is_file() and os.access(PERCHANCE, os.X_OK),
        "busy": PERCHANCE_LOCK.locked(),
        "launcher": str(PERCHANCE),
