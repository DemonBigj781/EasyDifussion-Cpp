...
def model_type_to_latent_family(model_type):
    """Map an identified diffusion checkpoint type to a city96 latent family."""
    model_type = str(model_type or "").lower()
    if model_type.startswith("sd_v1") or model_type.startswith("sd_v2") or model_type == "instruct_pix2pix":
        return "v1"
    if model_type.startswith("sd_xl") or model_type.startswith("playground_v2_5"):
        return "xl"
    if model_type.startswith("sd_v3"):
        return "v3"
    if model_type.startswith("flux") or model_type == "chroma":
        return "fx"
    if model_type.startswith("stable_cascade"):
        return "ca"
    return None
...
def identify_vae_latent_family(path):
    ...
    Flux/SD3 VAEs are distinguishable by their 16-channel tensor shapes. SD1
    and SDXL VAEs share the same architecture, so their directory/file naming
    is the authoritative discriminator, with SD1 as the conservative fallback.
    """
    normalized = str(path).replace("\\", "/").lower()
    if normalized.endswith((".safetensors", ".sft")):
        header = read_safetensors_header(path)
    elif normalized.endswith(".gguf"):
        header = read_gguf_header(path)
    else:
        raise ValueError(f"Unsupported VAE file type: {path}")

    decoder_shape = shape_of(header, "decoder.conv_in.weight")
    if decoder_shape is None:
        decoder_shape = shape_of(header, "first_stage_model.decoder.conv_in.weight")
    latent_channels = decoder_shape[1] if decoder_shape and len(decoder_shape) >= 2 else None

    if latent_channels == 16:
        if "sd3" in normalized or "stable-diffusion-3" in normalized:
            return "v3"
        return "fx"

    if any(token in normalized for token in ("/sdxl/", "sd_xl", "sdxl", "pony", "illustrious", "noobai")):
        return "xl"
    if any(token in normalized for token in ("/sd3/", "stable-diffusion-3")):
        return "v3"
