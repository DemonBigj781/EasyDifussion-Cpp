(function () {
    ...
    styleSheet.textContent = `
        ...
        .gallery-load-actions button:hover {
            background: var(--accent-color, #2168bf);
        }

        .gallery-image-wrapper:hover .gallery-delete-btn,
        .gallery-image-wrapper:hover .gallery-download-btn,
        .gallery-image-wrapper:hover .gallery-load-actions {
            opacity: 1;
        }
...
@media (hover: none), (pointer: coarse) {
    ...
    const companion = document.createElement('template')
    companion.innerHTML = window.loadRequiredPluginHTML('/plugins/core/gallery_plugin/gallery.tab.plugin.html')
    const appendTemplate = (selector, parent) => {
        const template = companion.content.querySelector(selector)
        if (!template || !parent) throw new Error(`Gallery companion is missing ${selector}`)
        parent.appendChild(template.content.cloneNode(true))
    }

    appendTemplate('#gallery-tab-template', document.querySelector('.tab-container'))
    appendTemplate('#gallery-content-template', document.querySelector('#tab-content-wrapper'))
    ...
    if (systemSettingsTable && !document.querySelector('#gallery-directory-setting')) {
        appendTemplate('#gallery-setting-template', systemSettingsTable)
    }

    appendTemplate('#gallery-lightbox-template', document.body)
    appendTemplate('#gallery-delete-confirm-template', document.body)
    ...
    async function galleryImageFile(imgData) {
        const response = await fetch(imgData.src, { cache: 'no-store' })
        if (!response.ok) throw new Error(`Could not load gallery image (${response.status})`)
        const blob = await response.blob()
        const filename = imgData.filename || `gallery-image-${Date.now()}.png`
    ...
    async function loadGalleryConfig(imgData) {
        if (typeof window.loadSetupFromImage !== 'function') {
            throw new Error('The image metadata loader is unavailable')
        }
        const file = await galleryImageFile(imgData)
        await window.loadSetupFromImage(file, false)
        document.querySelector('#tab-main')?.click()
    }
        ...
        if (galleryImages.length === 0) {
            ...
            if (needsRebuild) {
                ...
                galleryImages.forEach((imgData, index) => {
                    ...
                    loadActions.className = 'gallery-load-actions'
                    const loadImageBtn = document.createElement('button')
                    loadImageBtn.type = 'button'
                    loadImageBtn.className = 'gallery-load-image-btn'
                    loadImageBtn.textContent = 'Load image'
                    loadImageBtn.title = 'Load as the Initial Image'
                    loadImageBtn.addEventListener('click', async (event) => {
                        event.stopPropagation()
                        loadImageBtn.disabled = true
                        try {
                            await loadGalleryImage(imgData)
                        } catch (error) {
                            showGalleryActionMessage(error.message, true)
                        } finally {
                    ...
                    loadConfigBtn.className = 'gallery-load-config-btn'
                    loadConfigBtn.textContent = 'Load as config'
                    loadConfigBtn.title = 'Restore generation settings embedded in this image'
                    loadConfigBtn.addEventListener('click', async (event) => {
                        event.stopPropagation()
                        loadConfigBtn.disabled = true
                        try {
                            await loadGalleryConfig(imgData)
                        } catch (error) {
                            showGalleryActionMessage(error.message, true)
                        } finally {
                    ...
                    loadActions.append(loadImageBtn, loadConfigBtn)
                    wrapper.appendChild(loadActions)
