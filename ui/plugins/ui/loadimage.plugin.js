...
(function() { 
    ...
    function init() {
        if (document.querySelector("#load_image_btn")) {
            return;
        }

        const initImageButtons = document.querySelector("#init_image_buttons");
        const nativeBrowseInput = document.querySelector("#init_image");
        const nativeBrowseButton = nativeBrowseInput?.closest(".button");

        if (!initImageButtons || !nativeBrowseButton) {
            console.warn("Load Image Plugin: Could not find the Initial Image Browse control");
            return;
        }

        // Match Easy Diffusion's native Initial Image controls.
        const loadImageBtn = document.createElement("div");
        loadImageBtn.id = "load_image_btn";
        loadImageBtn.className = "button";
        loadImageBtn.title = "Load an image into the generated-images area";
        loadImageBtn.setAttribute("role", "button");
        loadImageBtn.setAttribute("tabindex", "0");
        loadImageBtn.innerHTML = '<i class="fa-solid fa-images"></i><span>Load Image</span>';

        // The transparent input fills the native-style button, like Browse.
        const fileInput = document.createElement('input');
        fileInput.type = 'file';
        fileInput.accept = 'image/*';
        fileInput.id = 'load_image_input';
        fileInput.setAttribute("aria-label", "Load an image into generated images");
        loadImageBtn.appendChild(fileInput);

        loadImageBtn.addEventListener("keydown", function(event) {
        ...
        buttonStack.id = "load-image-button-stack";
        buttonStack.append(nativeBrowseButton, loadImageBtn);
        initImageButtons.insertBefore(buttonStack, initImageButtons.firstChild);

        const style = document.createElement("style");
        style.id = "load-image-native-ui-css";
        style.textContent = `
            #load-image-button-stack {
                display: flex;
