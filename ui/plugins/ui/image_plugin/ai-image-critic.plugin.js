...
;(function () {
    ...
    panel.innerHTML = `
        ...
        <div class="collapsible-content settings-panel-entries">
            <p>Judge prompt adherence and image quality locally with llama.cpp. Select a complete vision bundle, or pair a standalone projector with its matching language GGUF.</p>
            <label for="${ID}-text-model">Language GGUF <small>(standalone projector only)</small></label>
            <select id="${ID}-text-model"></select>
            <label for="${ID}-vision-model">Vision bundle / projector</label>
            <select id="${ID}-vision-model"></select>
            <label for="${ID}-compute-mode">Compute</label>
            <select id="${ID}-compute-mode">
                <option value="gpu">GPU offload</option>
                <option value="cpu">CPU only</option>
            </select>
            <label for="${ID}-gpu-layers">GPU layers <small>(GPU mode)</small></label>
    ...
    const computeMode = document.getElementById(`${ID}-compute-mode`)
    ...
    computeMode.value = saved.computeMode ?? (gpuLayers.value === "0" ? "cpu" : "gpu")
    contextSize.value = saved.contextSize ?? "4096"

    function updateComputeMode() {
        const cpuOnly = computeMode.value === "cpu"
        gpuLayers.disabled = cpuOnly
        gpuLayers.title = cpuOnly
            ? "CPU-only mode disables all model, KV-cache, operation, and projector GPU offload."
            : "Maximum language-model layers to keep in VRAM."
    }
    ...
    ;[textModel, visionModel, computeMode, gpuLayers, contextSize].forEach((field) => {
        field.addEventListener("change", saveSettings)
    })
    computeMode.addEventListener("change", updateComputeMode)
    updateComputeMode()
    ...
    async function requestJson(url, options) {
        ...
        if (!response.ok) {
            ...
            const error = new Error(structured?.message || data.detail || `${response.status} ${response.statusText}`)
            error.debug = structured?.debug || {
    ...
    function fillSelect(select, values, selected, label) {
        ...
        values.forEach((entry) => {
            ...
            option.textContent = typeof entry === "string"
                ? entry
                : `${value}${entry.kind === "bundle" ? " — complete bundle" : entry.kind === "bundle_projector" ? " — bundled projector" : ""}${entry.usable ? "" : " — incompatible"}`
            option.title = typeof entry === "string" ? value : entry.detail
            option.dataset.usable = typeof entry === "string" || entry.usable ? "true" : "false"
            option.dataset.kind = typeof entry === "string" ? "language" : entry.kind
            select.appendChild(option)
        })
    ...
    function updateModelPairControls() {
        const kind = visionModel.selectedOptions[0]?.dataset.kind
        const needsLanguageModel = kind === "projector"
        textModel.disabled = !needsLanguageModel
        textModel.title = needsLanguageModel
            ? "Select the language GGUF made for this standalone projector."
            : "The selected vision bundle supplies its own language model."
    }
    ...
    visionModel.addEventListener("change", updateModelPairControls)
    ...
    function debugMarkup(debug) {
        if (!debug || typeof debug !== "object") return ""
        const parsed = debug.parsed_response && typeof debug.parsed_response === "object"
            ? JSON.stringify(debug.parsed_response, null, 2)
            : ""
        const metadata = Object.entries(debug)
            .filter(([key]) => !["raw_response", "parsed_response", "server_log_tail", "response_body"].includes(key))
            .map(([key, value]) => `${key}: ${value}`)
            .join("\n")
        const textBlock = (label, value) => value === undefined || value === null || value === "" ? "" : `
            <h5 style="margin:12px 0 5px">${label}</h5>
            <textarea readonly spellcheck="false" style="box-sizing:border-box;width:100%;min-height:110px;max-height:320px;white-space:pre;font-family:monospace">${escapeHtml(value)}</textarea>`
        return `
            <details open style="margin-top:18px;border-top:1px solid rgba(127,127,127,.35);padding-top:12px">
                <summary style="cursor:pointer;font-weight:bold">Debug data (full local response)</summary>
                ${textBlock("Request / model", metadata)}
                ${textBlock("Raw model response", debug.raw_response)}
                ${textBlock("Parsed model JSON", parsed)}
                ${textBlock("llama-server log tail", debug.server_log_tail)}
                ${textBlock("HTTP response body", debug.response_body)}
            </details>`
    }
    ...
    function showError(error) {
        ...
        modal.innerHTML = `
            <div style="background:var(--background-color2,#1e1e2e);color:var(--text-color,#eee);border-radius:10px;padding:22px;max-width:900px;width:100%;max-height:88vh;overflow:auto;box-shadow:0 8px 32px rgba(0,0,0,.7)">
                <div style="display:flex;justify-content:space-between;gap:12px;align-items:center">
                    <h3 style="margin:0">AI Image Critic failed</h3>
    ...
    function showReport(report) {
        ...
        const adherence = escapeHtml(String(report.prompt_adherence || "not_provided").replace(/_/g, " "))
        const adherenceSummary = escapeHtml(report.prompt_adherence_summary)
        modal.innerHTML = `
            <div style="background:var(--background-color2,#1e1e2e);color:var(--text-color,#eee);border-radius:10px;padding:22px;max-width:760px;width:100%;max-height:88vh;overflow:auto;box-shadow:0 8px 32px rgba(0,0,0,.7)">
                ...
                <h4>Prompt adherence: ${adherence}</h4>
                <p>${adherenceSummary || "No generation prompt was available for comparison."}</p>
                <h4>Issues</h4>
    ...
    async function analyzeImage(request, image, _event, tools) {
        ...
        try {
            const report = await requestJson("/ai-critic/analyze", {
                ...
                body: JSON.stringify({
                    ...
                    gpu_layers: computeMode.value === "cpu"
                        ? 0
                        : Number.parseInt(gpuLayers.value, 10) || 0,
            ...
            showError(error)
            notify("AI critic analysis failed. Full details are open in the critic window.", true)
        } finally {
