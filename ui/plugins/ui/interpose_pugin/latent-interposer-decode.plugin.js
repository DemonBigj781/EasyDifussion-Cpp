// Native sdkit3 checkpoint -> VAE latent conversion.
// Independent Easy Diffusion settings plugin.

;(function () {
    "use strict"
    if (window.__latentInterposerDecodePluginLoaded) return
    window.__latentInterposerDecodePluginLoaded = true

    const PANEL_ID = "sdkit3-decode-interpose-panel"
    const STATE_KEY = "easy-diffusion-latent-interposer-decode-v2"
    ...
    if (!editor?.parentNode || typeof ModelDropdown !== "function" || typeof PLUGINS !== "object") {
        console.error("Decode Interpose: required Easy Diffusion UI APIs were not found")
        return
    }
    ...
    panel.dataset.featureKeys = "backend_sdkit3"
        panel.innerHTML = window.loadRequiredPluginHTML("/plugins/core/interpose_pugin/latent-interposer-decode.plugin.html")

    const encodePanel = document.getElementById("sdkit3-encode-interpose-panel")
    if (encodePanel) encodePanel.after(panel)
    ...
    const enabled = byId("latent-interposer-decode-enabled")
    const source = byId("latent-interposer-decode-source")
    const destination = byId("latent-interposer-decode-destination")
    const direction = byId("latent-interposer-decode-direction")
    const status = byId("latent-interposer-decode-status")
    const model = new ModelDropdown(byId("latent-interposer-decode-model"), "latent-interposer", "None")
    ...
    function refresh() {
        const origin = checkpointFamily()
        const target = vaeFamily(origin)
        ...
        if (available) {
            ...
            status.textContent = same
                ? "Checkpoint already matches Custom VAE; decode conversion is not needed."
    ...
    enabled.checked = Boolean(state.enabled ?? legacy.decodeInterposerEnabled)
    ...
    PLUGINS.TASK_CREATE.push(function (event) {
        ...
        event.reqBody.latent_interposer_decode_model = model.value
        saveState()
    })
