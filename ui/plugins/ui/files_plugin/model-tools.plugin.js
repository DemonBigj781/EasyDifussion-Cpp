; (function () {
    ...
    let ggufUI = `
        <div class="panel-box gguf-converter-grid">
            <div>
                <label for="gguf-source-folder"><b>Downloaded model source</b></label><br>
                <small>Select a checkpoint, or enter a Hugging Face/Diffusers folder relative to the models directory.</small>
            </div>
            <div>
                <input id="gguf-source-folder" type="text" spellcheck="false" autocomplete="off"
                    list="gguf-source-options" placeholder="checkpoints/sdxl/model.safetensors">
                <datalist id="gguf-source-options"></datalist>
                <small id="gguf-source-summary">Scanning downloaded checkpoints…</small>
            </div>
            ...
            <select id="gguf-output-type">
                ...
                <option value="q5_0">Q5_0 (native checkpoints)</option>
                <option value="q5_1">Q5_1 (native checkpoints)</option>
                <option value="q4_0">Q4_0 (native checkpoints)</option>
                <option value="q4_1">Q4_1 (native checkpoints)</option>
    ...
    async function initGgufUI() {
        ...
        try {
            readiness = await fetch("/model-tools/gguf/readiness", { cache: "no-store" }).then(jsonResponse)
        ...
        try {
            const sourceData = await fetch("/model-tools/gguf/sources", { cache: "no-store" }).then(jsonResponse)
            ...
            sourceSummary.textContent = sourceData.count
                ? `${sourceData.count} downloaded checkpoint${sourceData.count === 1 ? "" : "s"} available.`
                : "No files found under models/checkpoints; a compatible model folder can still be entered."
        } catch (error) {
            sourceSummary.textContent = `Could not scan downloaded checkpoints: ${error.message}`
        }

        sourceEl.addEventListener("change", () => {
            const source = discoveredSources.get(sourceEl.value.trim())
            if (source && !outputEl.value.trim()) outputEl.value = source.name
            if (!readiness) return
        ...
        button.addEventListener("click", async () => {
            ...
            if (!source) {
                logEl.textContent = "Choose a downloaded checkpoint or a compatible model folder."
                sourceEl.focus()
                return
            }
            button.disabled = true
            logEl.textContent = "Starting GGUF conversion…"
            try {
