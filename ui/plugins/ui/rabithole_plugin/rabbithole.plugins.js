...
function load() {
	...
	try {
		...
		console.warn('Rabbit Hole ignored invalid saved settings.', error);
		tempSettings = {};
	}
  ...
  if (event.isComposing || event.keyCode === 229) {
	...
	async function rhLoadModels() {
		try {
			...
			if (!modelOptions && Array.isArray(getmodels.models)) {
				modelOptions = {
					'stable-diffusion': [],
					'gfpgan': [],
					'hypernetwork': [],
				...
				getmodels.models.forEach(model => {
					const type = Array.isArray(model.tags) ? model.tags[0] : model.type;
					if (modelOptions[type] && model.model) modelOptions[type].push(model.model);
				});
			}
			if (!modelOptions) throw new Error('Unsupported /get/models response.');
			...
			const loraHeading = document.getElementById('loraHeading');
			if (modelHeading) modelHeading.outerHTML = modelGroupHTML;
			if (loraHeading) loraHeading.outerHTML = loraGroupHTML;
...
function addRabbitHoleSettings(){
	...
	if (typeof window.loadRequiredPluginHTML === 'function') {
		try {
			compatibilityHtml = window.loadRequiredPluginHTML('/plugins/core/rabithole_plugin/rabbithole.plugin.html');
		} catch (error) {
	...
	if (editorSettings?.parentNode) {
		...
		document.getElementById('editor')?.appendChild(rabbitHoleSettings);
	}
