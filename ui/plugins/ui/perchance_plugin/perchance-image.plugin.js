(function () {
    ...
    async function loadPreview(image, card, sourceUrl) {
        let lastError = new Error("Preview could not be loaded.")
        for (let attempt = 0; attempt < PREVIEW_LOAD_ATTEMPTS; attempt += 1) {
            let objectUrl = ""
            try {
                const separator = sourceUrl.includes("?") ? "&" : "?"
                const response = await fetch(
                    `${sourceUrl}${separator}preview=${Date.now()}-${attempt}`,
                    { cache: "no-store" },
                )
                if (!response.ok) throw new Error(`Preview request failed with status ${response.status}.`)
                const contentType = response.headers.get("content-type") || ""
                if (!contentType.toLowerCase().startsWith("image/")) {
                    throw new Error("Preview request returned non-image content.")
                }
                const blob = await response.blob()
                if (!blob.size) throw new Error("Preview request returned an empty image.")

                objectUrl = URL.createObjectURL(blob)
                image.src = objectUrl
                await image.decode()
                if (!image.naturalWidth || !image.naturalHeight) {
                    throw new Error("Preview image decoded without dimensions.")
                }
                ...
                if (attempt + 1 < PREVIEW_LOAD_ATTEMPTS) {
    ...
    function attach() {
        ...
        function renderImages(items, labelText) {
            ...
            grid.querySelectorAll("img[data-preview-object-url]").forEach((image) => {
            ...
            images.forEach((item, index) => {
                ...
                card.style.cssText = "margin:0;min-width:0;background:rgba(0,0,0,.12);border-radius:6px;"
                const image = document.createElement("img")
                image.alt = `Generated by Perchance (${index + 1} of ${images.length})`
                image.style.cssText = "display:block;width:100%;min-height:180px;max-height:640px;object-fit:contain;border-radius:6px;visibility:hidden;"
