# DiffUser.cpp layout and planning audit

This audit is the status map for `source/DiffUser.cpp`. It prevents an empty scaffold,
...
- old DiffUser.cpp root backend directories -> feature/backend layers. The unused
  `library/backends/` notes/compatibility tree was removed.
- old DiffUser.cpp root `include/api` and `src` -> `library/include/api` and
  `library/src`.
- old `DiffUser.cpp/cuda/attention/*` ownership -> each attention feature's
